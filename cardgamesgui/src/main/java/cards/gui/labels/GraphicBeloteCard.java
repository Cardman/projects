...
public class GraphicBeloteCard extends PaintableLabel {
    ...
    public static BufferedImage getDefaultImage(String _lg, CardBelote _card) {
        ...
        if(!_card.isCharacter()) {
            ...
            for(byte b=0;b<2;b++) {
                ...
                if(_card.getNomFigure() == CardChar.JACK) {
    ...
    private static void dessinerPetitSymbole(CardBelote _card, Graphics2D _g,int _x,int _y) {
    ...
    private static void dessinerGrandSymbole(CardBelote _card, Graphics2D _g,int _x,int _y) {
        Suit couleur_=_card.couleur();
        if(couleur_ == Suit.HEART) {
            _g.setColor(Color.RED);
