...
public abstract class AbsScrollCustomCombo<T> implements ScrollCustomComboInt {
    ...
    private final AbsCustCellRenderGeneImpl<T> adaptCustCellRenderString;

    private boolean enabled = true;
    private boolean visiblePop;
    protected AbsScrollCustomCombo(AbsCompoFactory _compo, AbstractImageFactory _img, AbsActionListenerAct _act, AbsCustCellRenderGeneImpl<T> _rend) {
        popupMenu = _compo.newAbsPopupMenu();
        popupMenu.setFocusable(false);
        popupMenu.setVisible(false);
        adaptCustCellRenderString = _rend;
        list = new DefScrollCustomGraphicList<T>(_compo, _img, adaptCustCellRenderString, true);
        ...
        selected.addFocusListener(new RefreshComboFocusEvent<T>(this));
        selected.addMouseListener(_act,new SelectingComboListEvent<T>(this));
        pseudoButton_.addMouseListener(_act,new SelectingComboListEvent<T>(this));
        ...
        list.getElements().addMouseListener(_act,new MoveComboHideAfterMouseEvent<T>(this));
    }
    ...
    public ScrollCustomGraphicList<T> getList() {
        return list;
    }
    ...
    public void text() {
        RowGraphicList<T> r_ = getList().getRow(getSelectedIndex());
        if (r_ == null) {
            update("");
            return;
        }
        T i_ = r_.getInfo();
        update(adaptCustCellRenderString.convert(i_));
    }
