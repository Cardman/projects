...
public final class FileOpenFrameTest extends EquallableGuiFctUtil {
    ...
    public void type() {
        ...
        pr_.getFileCoreStream().newFile("tmp").mkdirs();
        pr_.setCurrentPath("/tmp");
        MockBinFact.write(pr_.getStreams().getBinFact(),"tex1","inner",false);
        MockBinFact.write(pr_.getStreams().getBinFact(),"txt2","inner",false);
        pr_.getFileCoreStream().newFile("tmp1").mkdirs();
        pr_.getFileCoreStream().newFile("tmp2").mkdirs();
        FileOpenFrame open_ = new FileOpenFrame(pr_,new ConcreteBoolean(false));
    ...
    public void abs2() {
        ...
        pr_.getFileCoreStream().newFile("tmp1").mkdirs();
        pr_.getFileCoreStream().newFile("tmp2").mkdirs();
        MockBinFact.write(pr_.getStreams().getBinFact(),"tmp1/txt1","inner",false);
        MockBinFact.write(pr_.getStreams().getBinFact(),"tmp1/txt2","inner",false);
        FileOpenFrame open_ = new FileOpenFrame(pr_,new ConcreteBoolean(false));
        FileOpenFrame.setFileSaveDialogByFrame(true,"/tmp",open_,new DefButtonsOpenPanelAct(new ContinueFileSample()));
        assertTrue(open_.getFrame().isVisible());
    ...
    public void search1() {
        ...
        pr_.getFileCoreStream().newFile("tmp2").mkdirs();
        MockBinFact.write(pr_.getStreams().getBinFact(),"tmp1/sub/txt1","inner",false);
        MockBinFact.write(pr_.getStreams().getBinFact(),"tmp1/sub/txt2","inner",false);
        MockBinFact.write(pr_.getStreams().getBinFact(),"tmp1/txt1","inner",false);
        MockBinFact.write(pr_.getStreams().getBinFact(),"tmp1/txt2","inner",false);
        MockBinFact.write(pr_.getStreams().getBinFact(),"tmp2/txt","inner",false);
        FileOpenFrame open_ = new FileOpenFrame(pr_,new ConcreteBoolean(false));
        FileOpenFrame.setFileSaveDialogByFrame(true,"/tmp",open_,new DefButtonsOpenPanelAct(new ContinueFileSample()));
        assertTrue(open_.getFrame().isVisible());
...
//        ((MockCompoFactory)pr_.getCompoFactory()).getLater().clear();
    ...
    public void inputFrame3() {
        MockProgramInfosSecSample pr_ = init();
        updateFileOpen(pr_);
        pr_.getTranslations().getMapping().getVal(StringUtil.EN).getMapping().getVal(MessagesGuiFct.GUI).getMapping().addEntry(MessagesGuiFct.CONFIRM,MessagesConfirmDialog.en());
        ...
        MockPlainButton c_ = (MockPlainButton) ((MockPanel)saver_.getFileDialogContent().getButtons()).getComponent(0);
