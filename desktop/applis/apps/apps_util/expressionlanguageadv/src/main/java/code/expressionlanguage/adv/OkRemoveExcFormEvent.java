...
public final class OkRemoveExcFormEvent implements AbsActionListener {
    ...
    @Override
    public void action() {
        window.getCurrentResult().getContext().excList().remove(window.getCurrentResult().getContext().notNullBuild(frameExcFormContent.getExact().isSelected(), frameExcFormContent.getClName().getText()));
