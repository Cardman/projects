...
public final class OkWpFormEvent implements AbsActionListener {
    ...
    public void action() {
        ...
        if (wp_ == null) {
            ...
            wp_ = currentResult.watch(window.getFramePoints().getFrameWpFormContent().getTrueField().isSelected(),new SynthFieldInfo(new ClassField(clName_, fieldName_),r_, window.getFramePoints().getFrameWpFormContent().getTrueField().isSelected(), null));
