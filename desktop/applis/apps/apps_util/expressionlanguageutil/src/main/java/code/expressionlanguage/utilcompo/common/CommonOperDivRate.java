...
public final class CommonOperDivRate implements CommonOperSymbol {
    ...
    public CommonOperDivRate(MathAdvAliases _m, boolean _allints) {
        aliases = _m;
        all = _allints;
    }
    @Override
    public Struct calculateOperator(Struct _first, Struct _second, byte _cast) {
        ...
        if (all) {
            return new RateStruct(new Rate(LgInt.divide(quot_.intPart(),div_.intPart())),aliases);
        }
        return new RateStruct(Rate.divide(((RateStruct)_first).getRate(),((RateStruct)_second).getRate()),aliases);
    }
}
