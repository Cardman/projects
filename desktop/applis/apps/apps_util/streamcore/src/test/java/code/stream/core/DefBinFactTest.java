...
public class DefBinFactTest extends EquallableStreamCoreUtil {
    ...
    public void t3() {
        MockFileSet set_ = fileSet(0,new long[0],"/");
        new DefBinFact(new MockBinFactory(set_, new MockTrueRand())).writeFile("abc", StringUtil.encode("abc"), false);
        assertEq("abc",StringUtil.decode(set_.getFiles().getVal("abc").getContent()));
        new DefBinFact(new MockBinFactory(set_, new MockFalseRand())).writeFile("abc", StringUtil.encode("abc"), false);
    }
