...
public final class FightHelpBeanAbilitiesChangingTypesDamageTest extends InitDbFightHelp {
    @Test
    public void movesTypesDefWeatherInitTest() {
        StringList ls_ = FightHelpBean.abilitiesChangingTypesDamageInit(db().getData());
        assertEq(1,ls_.size());
        assertTrue(StringUtil.contains(ls_,M_DAM));
    }
    @Test
    public void init() {
        assertSizeEq(1,callFightHelpBeanAbilitiesChangingTypesDamageGet(bean(db())));
    }
    @Test
    public void tr() {
        assertEq(M_DAM_TR,callFightHelpBeanGetTrAbilitiesChangingTypesDamage(bean(db()),0));
    }
    @Test
    public void cl1() {
    ...
    private String clickId() {
        NaSt b_ = bean(db());
        callFightHelpBeanClickAbilitiesChangingTypesDamage(b_,0);
        return getValAbilityId(b_);
    }
    private static FacadeGame db() {
        FacadeGame f_ = facade();
        AbilityData t_ = Instances.newAbilityData();
        t_.setChgtTypeByDamage(true);
