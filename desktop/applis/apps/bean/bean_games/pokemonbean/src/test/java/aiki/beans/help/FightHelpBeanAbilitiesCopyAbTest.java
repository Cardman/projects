...
public final class FightHelpBeanAbilitiesCopyAbTest extends InitDbFightHelp {
    @Test
    public void movesTypesDefWeatherInitTest() {
        StringList ls_ = FightHelpBean.abilitiesCopyAbInit(db().getData());
        assertEq(1,ls_.size());
        assertTrue(StringUtil.contains(ls_,M_DAM));
    }
    @Test
    public void init() {
        assertSizeEq(1,callFightHelpBeanAbilitiesCopyAbGet(bean(db())));
    }
    @Test
    public void tr() {
        assertEq(M_DAM_TR,callFightHelpBeanGetTrAbilitiesCopyAb(bean(db()),0));
    }
    @Test
    public void cl1() {
    ...
    private String clickId() {
        NaSt b_ = bean(db());
        callFightHelpBeanClickAbilitiesCopyAb(b_,0);
