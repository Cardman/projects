...
public final class FightHelpBeanMovesIgnEvaTest extends InitDbFightHelp {
    @Test
    public void movesTypesDefWeatherInitTest() {
        StringList ls_ = FightHelpBean.movesIgnEvaInit(db().getData());
        assertEq(1,ls_.size());
        assertTrue(StringUtil.contains(ls_,M_DAM));
    }
    @Test
    public void init() {
        assertSizeEq(1,callFightHelpBeanMovesIgnEvaGet(bean(db())));
    }
    @Test
    public void tr() {
        assertEq(M_DAM_TR,callFightHelpBeanGetTrMovesIgnEva(bean(db()),0));
    }
    @Test
    public void cl1() {
    ...
    private String click() {
        NaSt b_ = bean(db());
        return toStr(callFightHelpBeanClickMovesIgnEva(b_,0));
    }
    private String clickId() {
        NaSt b_ = bean(db());
        callFightHelpBeanClickMovesIgnEva(b_,0);
