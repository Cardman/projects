...
public final class FightHelpBeanMovesMirrorTest extends InitDbFightHelp {
    @Test
    public void movesTypesDefWeatherInitTest() {
        StringList ls_ = FightHelpBean.movesMirrorInit(db().getData());
        assertEq(1,ls_.size());
        assertTrue(StringUtil.contains(ls_,M_DAM));
    }
    @Test
    public void init() {
        assertSizeEq(1,callFightHelpBeanMovesMirrorGet(bean(db())));
    }
    @Test
    public void tr() {
        assertEq(M_DAM_TR,callFightHelpBeanGetTrMovesMirror(bean(db()),0));
    }
    @Test
    public void cl1() {
    ...
    private String click() {
        NaSt b_ = bean(db());
        return toStr(callFightHelpBeanClickMovesMirror(b_,0));
    }
    private String clickId() {
        NaSt b_ = bean(db());
        callFightHelpBeanClickMovesMirror(b_,0);
    ...
    private static FacadeGame db() {
        ...
        e_.setPointViewChangement(PointViewChangementType.MIRROR_AGAINST_THROWER);
