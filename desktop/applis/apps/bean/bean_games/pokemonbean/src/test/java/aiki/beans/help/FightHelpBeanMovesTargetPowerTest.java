...
public final class FightHelpBeanMovesTargetPowerTest extends InitDbFightHelp {
    @Test
    public void movesTypesDefWeatherInitTest() {
        StringList ls_ = FightHelpBean.movesTargetPowerInit(db().getData());
        assertEq(1,ls_.size());
        assertTrue(StringUtil.contains(ls_,M_DAM));
    }
    @Test
    public void init() {
        assertSizeEq(1,callFightHelpBeanMovesTargetPowerGet(bean(db())));
    }
    @Test
    public void tr() {
        assertEq(M_DAM_TR,callFightHelpBeanGetTrMovesTargetPower(bean(db()),0));
    }
    @Test
    public void cl1() {
    ...
    private String click() {
        NaSt b_ = bean(db());
        return toStr(callFightHelpBeanClickMovesTargetPower(b_,0));
    }
    private String clickId() {
        NaSt b_ = bean(db());
        callFightHelpBeanClickMovesTargetPower(b_,0);
