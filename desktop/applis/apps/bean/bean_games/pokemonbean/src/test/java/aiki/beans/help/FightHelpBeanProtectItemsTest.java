...
public final class FightHelpBeanProtectItemsTest extends InitDbFightHelp {
    @Test
    public void movesTypesDefWeatherInitTest() {
        StringList ls_ = FightHelpBean.protectItemsInit(db().getData());
        assertEq(1,ls_.size());
        assertTrue(StringUtil.contains(ls_,M_DAM));
    }
    @Test
    public void init() {
        assertSizeEq(1,callFightHelpBeanProtectItemsGet(bean(db())));
    }
    @Test
    public void tr() {
        assertEq(M_DAM_TR,callFightHelpBeanGetTrProtectItems(bean(db()),0));
    }
    @Test
    public void cl1() {
    ...
    private String click() {
        NaSt b_ = bean(db());
        return toStr(callFightHelpBeanClickProtectItems(b_,0));
    }
