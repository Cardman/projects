...
public final class ExecChoiceFctOperation extends ExecSettableCallFctOperation {
    ...
    public static Argument prep(ContextEl _conf, StackCall _stack, Argument _previous, CustList<ExecOperationInfo> _infos, ExecTypeFunctionInst _ins) {
        ExecFormattedRootBlock formattedType_ = _ins.getInst().getFormattedType();
        Argument prev_ = new Argument(ExecFieldTemplates.getParent(_ins.getInst().getAnc(), _previous.getStruct(), _conf, _stack));
        Argument res_;
        if (_conf.callsOrException(_stack)) {
            res_ = new Argument();
        } else {
            res_ = new MethodParamChecker(_ins.getPair(), fetchFormattedArgs(_conf, _stack, prev_.getStruct(), _ins, _infos), MethodAccessKind.INSTANCE).checkParams(formattedType_, prev_, null, _conf, _stack);
        }
        return res_;
    }
