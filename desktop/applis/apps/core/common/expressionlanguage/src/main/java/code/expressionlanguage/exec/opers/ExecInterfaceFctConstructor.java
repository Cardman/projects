...
public final class ExecInterfaceFctConstructor extends ExecAbstractInvokingConstructor {
    ...
    public void calculate(IdMap<ExecOperationNode, ArgumentsPair> _nodes,
        ...
        if (getIndexChild() == 1) {
            ...
            new DefaultParamChecker(getPair(), fectchArgs(_nodes, lastType_, getNaturalVararg(),null,_conf,_stack), MethodAccessKind.INSTANCE, CallPrepareState.CTOR, InstancingStep.USING_SUPER).checkParams(superClass_, ref_, null, _conf, _stack);
        ...
        ExecFormattedRootBlock superClass_ = _stack.formatVarType(getFormattedType());
        String lastType_ = ExecInherits.quickFormat(superClass_, getLastType());
        new DefaultParamChecker(getPair(), fectchArgs(_nodes, lastType_, getNaturalVararg(),null,_conf,_stack), MethodAccessKind.INSTANCE, CallPrepareState.CTOR, InstancingStep.USING_SUPER).checkParams(superClass_, mainArgument_, null, _conf, _stack);
        Argument res_ = Argument.createVoid();
        setSimpleArgument(res_, _conf, _nodes, _stack);
    }
