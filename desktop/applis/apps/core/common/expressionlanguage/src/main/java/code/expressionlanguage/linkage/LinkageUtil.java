...
public final class LinkageUtil {
    ...
    private static void tryAppendParts(VariablesOffsets _vars, PartOffsetsClassMethodId _p) {
        ...
        _vars.addParts(_p.getSuperTypes());
    }
    ...
    private static void processForMutableIterativeLoopReport(VariablesOffsets _vars, ForMutableIterativeLoop _cond, Coverage _cov) {
        ...
        if (_vars.getLastStackElt().getIndexLoop() == 0) {
            if (rootInit_ != null) {
                ...
                buildNormalReport(_vars, _cov, rootInit_, in_);
                if (_vars.goesToProcess()) {
                    return;
                }
            }
        ...
        if (_vars.getLastStackElt().getIndexLoop() == 1) {
            if (rootExp_ != null) {
                ...
                buildNormalReport(_vars, _cov, rootExp_, in_);
                if (_vars.goesToProcess()) {
                    return;
                }
            }
        ...
        if (rootStep_ != null) {
            ...
            buildNormalReport(_vars, _cov, rootStep_, in_);
            if (_vars.goesToProcess()) {
                return;
            }
        }
    ...
    private static void processForMutableIterativeLoopError(VariablesOffsets _vars, ForMutableIterativeLoop _cond) {
        ...
        if (_vars.getLastStackElt().getIndexLoop() == 0) {
            ...
            if (rootInit_ != null) {
                int off_ = _cond.getInitOffset();
                buildNormErr(_vars, _cond, -1, rootInit_, off_, 0);
                if (_vars.goesToProcess()) {
                    return;
                }
            }
        ...
        if (_vars.getLastStackElt().getIndexLoop() == 1) {
            ...
            if (rootExp_ != null) {
                int off_ = _cond.getExpressionOffset();
                buildNormErr(_vars, _cond, -1, rootExp_, off_, 1);
                if (_vars.goesToProcess()) {
                    return;
                }
            }
        ...
        if (rootStep_ != null) {
            int off_ = _cond.getStepOffset();
            buildNormErr(_vars, _cond, -1, rootStep_, off_, 2);
            if (_vars.goesToProcess()) {
                return;
            }
        }
    ...
    private static void processLineError(VariablesOffsets _vars, Line _cond) {
        int blOffset_ = _cond.getExpressionOffset();
        OperationNode root_ = _cond.getRoot();
        buildNormErr(_vars, _cond, -1, root_, blOffset_, 0);
    }
    ...
    private static void appendSecondVar(VariablesOffsets _vars, ForEachTable _cond, String _keyWordVar) {
        if (StringUtil.quickEq(_cond.getClassNameSecond().trim(), _keyWordVar)) {
            ...
            _vars.addParts(_cond.getPartOffsetsSecond());
        }
    }
    ...
    private static void appendFirstVar(VariablesOffsets _vars, ForEachTable _cond, String _keyWordVar) {
        if (StringUtil.quickEq(_cond.getClassNameFirst().trim(), _keyWordVar)) {
            ...
            _vars.addParts(_cond.getPartOffsetsFirst());
        }
    }
    ...
    private static void buildEnumEltError(VariablesOffsets _vars, AbsBk _cond, AbstractInstancingOperation _inst, int _k, int _begin) {
        buildNormalError(_vars, _cond, _k, _inst, _begin, 0);
    }
    ...
    private static void processFieldBlockError(VariablesOffsets _vars, FieldBlock _cond) {
        ...
        OperationNode root_ = _cond.getRoot();
        buildNormErr(_vars, _cond, k_, root_, blOffset_, 0);
        if (_vars.goesToProcess()) {
            return;
        }
        _vars.getLastStackElt().setIndexAnnotationGroup(-1);
    ...
    private static void buildAnnotField(VariablesOffsets _vars, FieldBlock _cond, Coverage _cov) {
        ...
        annotReport(_vars, _cond, _cov, annotationsIndexes_, annotations_, roots_);
    }
    private static void buildAnnotFieldErr(VariablesOffsets _vars, FieldBlock _cond) {
        Ints annotationsIndexes_ = _cond.getAnnotationsIndexes();
        CustList<OperationNode> roots_ = _cond.getRoots();
        annotError(_vars, _cond, annotationsIndexes_, roots_);
    }

    private static void annotError(VariablesOffsets _vars, AbsBk _cond, Ints _annotationsIndexes, CustList<OperationNode> _roots) {
        int len_ = _annotationsIndexes.size();
        int j_ = _vars.getLastStackElt().getIndexAnnotation();
        for (int i = j_; i < len_; i++) {
            buildAnnotErr(_vars, _cond, -1, _annotationsIndexes, _roots, i);
            if (_vars.goesToProcess()) {
                return;
            }
    ...
    private static void processOverridableBlockReport(VariablesOffsets _vars, NamedFunctionBlock _cond, Coverage _cov) {
        ...
        if (!AbsBk.isOverBlock(_cond) || ((NamedCalledFunctionBlock)_cond).getKind() == MethodKind.OPERATOR) {
            if (k_ == -1) {
                ...
                _vars.addParts(_cond.getPartOffsetsReturn());
            }
    ...
    private static void buildNormErr(VariablesOffsets _vars, AbsBk _cond, int _k, OperationNode _root, int _begin, int _indexLoop) {
        buildNormalError(_vars, _cond, _k, _root, _begin, _indexLoop);
    }

    private static void buildNormalError(VariablesOffsets _vars, AbsBk _cond, int _k, OperationNode _root, int _begin, int _indexLoop) {
    ...
    private static void annotReport(VariablesOffsets _vars, AbsBk _cond, Coverage _cov, Ints _annotationsIndexes, StringList _annotations, CustList<OperationNode> _roots) {
        int len_ = _annotationsIndexes.size();
        int j_ = _vars.getLastStackElt().getIndexAnnotation();
        for (int i = j_; i < len_; i++) {
            LinkageStackElementIn in_ = buildAnnotLinkageReport(_cond, -1, _annotationsIndexes, _annotations, i);
            buildAnnotReport(_vars, _cov, _roots, i, in_);
            if (_vars.goesToProcess()) {
                return;
            }
            _vars.getLastStackElt().setIndexAnnotation(i + 1);
    ...
    private static void annotMethReport(VariablesOffsets _vars, AbsBk _cond, Coverage _cov, Ints _annotationsIndexes, StringList _annotations, CustList<OperationNode> _roots) {
        int len_ = _annotationsIndexes.size();
    ...
    private static void buildAnnotReport(VariablesOffsets _vars, Coverage _cov, CustList<OperationNode> _roots, int _i, LinkageStackElementIn _in) {
        OperationNode root_ = _roots.get(_i);
        buildCoverageReport(_vars, _cov, root_, _in);
    }
    ...
    private static void refPar(VariablesOffsets _vars, NamedFunctionBlock _cond, Coverage _cov, int _k) {
        ...
        for (int i = _k; i < len_; i++) {
            ...
            _vars.addParts(_cond.getPartOffsetsParams().get(i));
    ...
    private static void buildNormalReport(VariablesOffsets _vars, Coverage _cov, OperationNode _root, LinkageStackElementIn _in) {
        buildCoverageReport(_vars, _cov, _root, _in);
    }
    ...
    private static OperationNode loopReport(OperationNode _val, VariablesOffsets _vars, Coverage _cov, OperationNode _root, LinkageStackElementIn _in) {
        ...
        while (true) {
            ...
            if (parent_ == null) {
                getEnd(_vars, _root, addCover_, _in);
                _vars.getLastStackElt().setNullCurrent();
                return null;
            }
            int offsetEnd_ = getOffsetEnd(sum_, val_, parent_);
            processImplicit(_vars, offsetEnd_,val_, parent_);
            getEndTag(_vars, _root, fieldLength_, addCover_, val_, offsetEnd_);
            processUnaryRightOperations(_in,_vars, sum_, offsetEnd_, val_,parent_);
    ...
    private static void getEndTag(VariablesOffsets _vars, OperationNode _root, int _fieldLength, boolean _addCover, OperationNode _val, int _offsetEnd) {
        if (addTag(_fieldLength,_root,_val,_addCover)) {
            _vars.addPart(new PartOffset(ExportCst.END_SPAN,_offsetEnd));
        }
    }
    ...
    private static void getBeginOpReport(VariablesOffsets _vars, LinkageStackElementIn _in, OperationNode _root, OperationNode _curOp, boolean _addCover, AbstractCoverageResult _result, Coverage _cov) {
        ...
        if (addTag(fieldLength_, _root, _curOp, _addCover)) {
            ...
            _vars.addPart(new PartOffset(tag_,_in.getBeginBlock() + _curOp.getIndexInEl()));
        }
    }

    private static void getEnd(VariablesOffsets _vars, OperationNode _root, boolean _addCover, LinkageStackElementIn _in) {
        int fieldLength_ = _in.getFieldLength();
        if (addTag(fieldLength_,_root,_root,_addCover)) {
            int endBlock_ = _in.getEndBlock();
            _vars.addPart(new PartOffset(ExportCst.END_SPAN, endBlock_));
        }
    }
    ...
    private static void errorsBefore(VariablesOffsets _vars, int _sum, OperationNode _val) {
        MethodOperation par_ = _val.getParent();
        if (par_ == null) {
            errorAtRoot(_vars, _sum, _val);
        } else {
    ...
    private static void errorAtRoot(VariablesOffsets _vars, int _sum, OperationNode _val) {
        ...
        if (!errEmpt_.isEmpty()) {
            ...
            _vars.addPart(new PartOffset(ExportCst.END_ANCHOR,begin_+ _val.getOperations().getDelimiter().getLength()));
        } else {
    ...
    private static void processNamedFct(VariablesOffsets _vars, int _sum, OperationNode _val) {
        if (_val instanceof FctOperation||_val instanceof ChoiceFctOperation ||_val instanceof SuperFctOperation) {
            if (_val instanceof FctOperation) {
                _vars.addParts(((FctOperation)_val).getPartOffsets());
    ...
    private static void processInstances(VariablesOffsets _vars, int _sum, OperationNode _val) {
        ...
        if (_val instanceof DimensionArrayInstancing) {
            _vars.addParts(((DimensionArrayInstancing)_val).getPartOffsets());
        }
    ...
    private static void processLeafType(VariablesOffsets _vars, int _sum, OperationNode _val) {
        ...
        if (_val instanceof ForwardOperation) {
            ...
            _vars.addParts(((ForwardOperation)_val).getPartOffsets());
        }
        if (_val instanceof DefaultValueOperation) {
            _vars.addParts(((DefaultValueOperation)_val).getPartOffsets());
        }
        if (_val instanceof StaticInfoOperation) {
            _vars.addParts(((StaticInfoOperation)_val).getPartOffsets());
        }
        if (_val instanceof StaticAccessOperation) {
            ...
            _vars.addParts(((StaticAccessOperation)_val).getPartOffsets());
        }
        if (_val instanceof StaticCallAccessOperation) {
            ...
            _vars.addParts(((StaticCallAccessOperation)_val).getPartOffsets());
        }
    ...
    private static void processRichHeader(VariablesOffsets _vars, int _sum, OperationNode _val) {
        if (_val instanceof EnumValueOfOperation) {
            ...
            _vars.addParts(((EnumValueOfOperation)_val).getPartOffsets());
        }
        if (_val instanceof ValuesOperation) {
            ...
            _vars.addParts(((ValuesOperation)_val).getPartOffsets());
        }
        if (_val instanceof AbstractInvokingConstructor) {
            if (_val instanceof InterfaceInvokingConstructor) {
                addParts(_vars, ((AbstractInvokingConstructor)_val).getConstructor(),
                        ...
                        _val.getErrs(),_val.getErrs());
                _vars.addParts(((InterfaceInvokingConstructor)_val).getPartOffsets());
            } else if (_val instanceof InterfaceFctConstructor) {
                addParts(_vars, ((AbstractInvokingConstructor)_val).getConstructor(),
                        _sum + _val.getIndexInEl(),_vars.getKeyWords().getKeyWordInterfaces().length(),
                        _val.getErrs(),_val.getErrs());
                _vars.addParts(((InterfaceFctConstructor)_val).getPartOffsets());
            } else{
                addParts(_vars, ((AbstractInvokingConstructor)_val).getConstructor(),
    ...
    private static void processCast(VariablesOffsets _vars, int _sum, OperationNode _val) {
        if (_val instanceof CastOperation) {
            ...
            _vars.addParts(((CastOperation) _val).getPartOffsets());
        }
        if (_val instanceof ExplicitOperation) {
            ...
            _vars.addParts(((ExplicitOperation) _val).getPartOffsets());
        }
        if (_val instanceof ImplicitOperation) {
            ...
            _vars.addParts(((ImplicitOperation) _val).getPartOffsets());
        }
    }
    ...
    private static void middleError(LinkageStackElementIn _in,
        ...
        if (middleOper(_parent)) {
            ...
            if (testFct_ != null) {
                ...
                _vars.addPart(new PartOffset(ExportCst.anchorErr(StringUtil.join(l_,ExportCst.JOIN_ERR)), _offsetEnd));
                _vars.addPart(new PartOffset(ExportCst.END_ANCHOR, _offsetEnd +Math.max(len_,1)));
            } else {
    ...
    private static void processImplicit(VariablesOffsets _vars,
        ...
        if ((middleOperNotShortTernary(_parent) || _parent instanceof SemiAffectationOperation && ((SemiAffectationOperation) _parent).isPost()) && _curOp.getIndexChild() == 0) {
            tryAddMergedParts(_vars, _curOp.getResultClass().getFunction(), _off, new StringList(), new StringList());
        }
    }
    ...
    private static void processUnaryRightOperations(LinkageStackElementIn _in, VariablesOffsets _vars,
                                                    int _sum, int _offsetEnd, OperationNode _curOp, MethodOperation _parent) {
        if (_curOp.getIndexChild() == 0 &&_parent instanceof InstanceOfOperation) {
            ...
            _vars.addParts(((InstanceOfOperation)_parent).getPartOffsets());
        }
        if (_parent instanceof RangeOperation && _parent.getChildrenNodes().size() == 1) {
            ...
            if (!l_.isEmpty()) {
                _vars.addPart(new PartOffset(ExportCst.anchorErr(StringUtil.join(l_,ExportCst.JOIN_ERR)), _offsetEnd));
                _vars.addPart(new PartOffset(ExportCst.END_ANCHOR, _offsetEnd +3));
            } else {
