...
public abstract class ProcessMethodCommon extends EquallableElUtil {
    ...
    protected static Struct calculateError(String _class, MethodId _method, ContextEl _cont) {
        ...
        ProcessMethod.calculate(new CustomFoundMethod(NullStruct.NULL_VALUE, new ExecFormattedRootBlock(classBody_,_class), new ExecTypeFunction(classBody_, method_), p_), _cont, stackCall_);
    ...
    protected static Struct calculateNormal(String _class, MethodId _method, ContextEl _cont) {
        ...
        Parameters p_ = new Parameters();
        StackCall stackCall_ = StackCall.newInstance(InitPhase.NOTHING,_cont);
        Struct arg_ = ProcessMethod.calculate(new CustomFoundMethod(NullStruct.NULL_VALUE, new ExecFormattedRootBlock(classBody_, _class), new ExecTypeFunction(classBody_, method_), p_), _cont, stackCall_).getValue();
    ...
    protected static Struct instanceNormal(String _class, ConstructorId _id, ContextEl _cont) {
        ...
        ExecRootBlock type_ = _cont.getClasses().getClassBody(StringExpUtil.getIdFromAllTypes(_class));
        StackCall stackCall_ = StackCall.newInstance(InitPhase.NOTHING,_cont);
        Struct arg_ = ProcessMethod.calculate(new CustomFoundConstructor(_cont,new ExecFormattedRootBlock(type_, _class), null), _cont, stackCall_).getValue();
        assertNull(stackCall_.getCallingState());
        return arg_;
    }
...
//        feedParams(_args, _cont, ctor_, p_);
        StackCall stackCall_ = StackCall.newInstance(InitPhase.NOTHING,_cont);
        Struct arg_ = ProcessMethod.calculate(new CustomFoundConstructor(_cont,new ExecFormattedRootBlock(type_, _class), new ExecTypeFunction(type_, ctor_), "",-1, null, p_), _cont, stackCall_).getValue();
        assertNull(stackCall_.getCallingState());
        return arg_;
    }
