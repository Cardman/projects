...
public final class ProcessMethodIterableTableTest extends ProcessMethodCommon {
    ...
    public void instanceArgument1221Test() {
        ...
        xml_.append("  inst.add(8,1);\n");
        xml_.append("  inst.add(2,6);\n");
        xml_.append("  $foreach(Number f, Number s: inst){\n");
        xml_.append("   res += f.intValue()+s.intValue();\n");
        xml_.append("  }\n");
        xml_.append(" }\n");
        xml_.append("}\n");
    ...
    public void instanceArgument131Test() {
        ...
        field_ = getField(str_, new ClassField("pkg.Ex", "res"));
        assertEq(INTEGER, field_.getClassName(cont_));
        assertEq(50, ((NumberStruct)field_).intStruct());
    }
    @Test
    ...
    public void instanceArgument140Test() {
        ...
        xml_.append("$public $class pkg.Ex {\n");
        xml_.append(" $public CustTable<Number,Number> inst=$new CustTable<Number,Number>();\n");
        xml_.append(" $public $boolean res = (inst $instanceof CustTable<?Object,Object>);\n");
        xml_.append("}\n");
        files_.put("pkg/Ex", xml_.toString());
        files_.put(CUST_ITER_PATH, getCustomIterator());
        files_.put(CUST_LIST_PATH, getCustomList());
