...
public final class RendSelect extends RendParentBlock implements RendWithEl, RendReducableOperations {
    ...
    public void buildExpressionLanguage(Configuration _cont, RendDocumentBlock _doc, AnalyzingDoc _anaDoc, AnalyzedPageEl _page) {
        ...
        if (!groupId_.isEmpty()) {
            ...
            rId_.buildId(groupId_, off_,_doc, _anaDoc, _page);
        ...
        opsMap = RenderExpUtil.getAnalyzedOperations(map_, 0, _anaDoc, _page);
        ...
        if (!default_.isEmpty()) {
            ...
            opsDefault = RenderExpUtil.getAnalyzedOperations(concat_, 0, _anaDoc, _page);
            Mapping m_ = new Mapping();
            m_.setArg(_anaDoc.getCurrentRoot().getResultClass());
            if (!multiple) {
                m_.setParam(_page.getStandards().getAliasCharSequence());
