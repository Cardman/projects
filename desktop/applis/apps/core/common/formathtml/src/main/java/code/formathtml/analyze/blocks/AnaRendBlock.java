...
//    static final String CALL_METHOD = "$";
    ...
    public static AnaRendDocumentBlock newRendDocumentBlock(Document _doc, String _docText, AnalyzedPageEl _primTypes, AnalyzingDoc _anaDoc, AdvFileEscapedCalc _es, FileBlock _fileBl, DocumentResult _chs) {
        RendKeyWords rend_ = _anaDoc.getRendKeyWords();
        Element documentElement_ = _doc.getDocumentElement();
        Node curNode_ = documentElement_;
        AnaRendDocumentBlock out_ = new AnaRendDocumentBlock(documentElement_, 0, _es, _fileBl, _chs.getEscaped());
        int indexGlobal_ = indexOfBeginNode(curNode_, _docText, _chs.getFirstIndex());
...
//            bl_.stringParts.addAllElts(RendSplitExpressionUtil.itText(_begin,_elt.getTextContent(),_primTypes).getStringParts());
    ...
    private static AnaRendBlock newRendBlock(int _begin, AnaRendParentBlock _curParent, String _prefix, Node _elt, String _docText, AnalyzedPageEl _primTypes, RendKeyWords _rendKeyWords) {
        ...
        if (StringUtil.quickEq(tagName_, StringUtil.concat(_prefix, _rendKeyWords.getKeyWordFor()))) {
            return keyWordFor(_begin, _primTypes.getPrimTypes(), _rendKeyWords, elt_, attr_);
        }
    ...
    private static AnaRendParentBlock stdKeys(int _begin, AnaRendParentBlock _curParent, AnalyzedPageEl _primTypes, String _prefix, RendKeyWords _rendKeyWords, Element _elt, StringMap<AttributePart> _attr) {
        ...
        if (StringUtil.quickEq(tagName_, StringUtil.concat(_prefix, _rendKeyWords.getKeyWordCatch()))) {
            return keyWordCatch(_begin,_primTypes, _rendKeyWords, _elt, _attr);
        }
    ...
    private static AnaRendAbstractCatchEval keyWordCatch(int _begin, AnalyzedPageEl _primTypes, RendKeyWords _rendKeyWords, Element _elt, StringMap<AttributePart> _attr) {
        ...
        if (!_elt.hasAttribute(_rendKeyWords.getAttrClassName()) && !_elt.hasAttribute(_rendKeyWords.getAttrValue())) {
