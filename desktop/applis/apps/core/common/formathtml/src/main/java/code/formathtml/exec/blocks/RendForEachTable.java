...
public final class RendForEachTable extends RendParentBlock implements RendLoop, RendWithEl {
    ...
    public void processEl(Configuration _cont, BeanLgNames _stds, ContextEl _ctx, StackCall _stack, RendStackCall _rendStack) {
        ...
        Argument arg_ = iteratorMultTable(its_,_cont, _stds, _ctx, _stack, _rendStack);
        if (_ctx.callsOrException(_stack)) {
            return;
        }
    ...
    Struct processLoop(Configuration _conf, BeanLgNames _advStandards, ContextEl _ctx, StackCall _stackCall, RendStackCall _rendStackCall) {
    ...
    public void processLastElementLoop(Configuration _conf, BeanLgNames _advStandards, ContextEl _ctx, RendLoopBlockStack _loopBlock, StackCall _stack, RendStackCall _rendStack) {
        ...
        ConditionReturn has_ = iteratorHasNext(_conf, _advStandards, _ctx, _loopBlock, _stack, _rendStack);
    ...
    public void incrementLoop(Configuration _conf, RendLoopBlockStack _l,
                              StringMap<LoopVariable> _vars, StringMap<AbstractWrapper> _varsInfos, BeanLgNames _advStandards, ContextEl _ctx, StackCall _stackCall, RendStackCall _rendStackCall) {
        _l.setIndex(_l.getIndex() + 1);
        Struct iterator_ = _l.getStructIterator();
        ImportingPage call_ = _rendStackCall.getLastPage();
        Argument nextPair_ = nextPair(iterator_,_conf, _advStandards, _ctx, _stackCall, _rendStackCall);
        if (_ctx.callsOrException(_stackCall)) {
            return;
        }
        Struct value_ = nextPair_.getStruct();
        Argument arg_ = first(value_,_conf, _advStandards, _ctx, _stackCall, _rendStackCall);
        if (_ctx.callsOrException(_stackCall)) {
            return;
        }
        if (!ExecTemplates.checkQuick(importedClassNameFirst, Argument.getNullableValue(arg_).getStruct().getClassName(_ctx), _ctx, _stackCall)) {
            return;
        }
        ...
        arg_ = second(value_,_conf, _advStandards, _ctx, _stackCall, _rendStackCall);
        if (_ctx.callsOrException(_stackCall)) {
            return;
        }
        if (!ExecTemplates.checkQuick(importedClassNameSecond, Argument.getNullableValue(arg_).getStruct().getClassName(_ctx), _ctx, _stackCall)) {
            return;
        }
    ...
    private ConditionReturn iteratorHasNext(Configuration _conf, BeanLgNames _advStandards, ContextEl _ctx, RendLoopBlockStack _rendLastStack, StackCall _stackCall, RendStackCall _rendStackCall) {
        Struct strIter_ = _rendLastStack.getStructIterator();
        Argument arg_ = hasNextPair(strIter_,_conf, _advStandards, _ctx, _stackCall, _rendStackCall);
        if (_ctx.callsOrException(_stackCall)) {
