...
public class RendSemiAffectationCustOperation extends RendSemiAffectationOperation {
    ...
    private Argument endCalculate(IdMap<RendDynOperationNode, ArgumentsPair> _nodes, Argument _stored, Argument _res, BeanLgNames _advStandards, ContextEl _context, RendStackCall _rendStackCall) {
        ...
        ExecStaticPostEltContent staticPostEltContent_ = getStaticPostEltContent();
        if (settable_ instanceof RendStdRefVariableOperation) {
            arg_ = ((RendStdRefVariableOperation)settable_).endCalculate(_nodes, staticPostEltContent_.isPost(), _stored, _res, _advStandards, _context, _rendStackCall);
        }
        if (settable_ instanceof RendSettableFieldOperation) {
            arg_ = ((RendSettableFieldOperation)settable_).endCalculate(_nodes, staticPostEltContent_.isPost(), _stored, _res, _advStandards, _context, _rendStackCall);
        }
        if (settable_ instanceof RendCustArrOperation) {
            arg_ = ((RendCustArrOperation)settable_).endCalculate(_nodes, staticPostEltContent_.isPost(), _stored, _res, _advStandards, _context, _rendStackCall);
        }
        if (settable_ instanceof RendArrOperation) {
            arg_ = ((RendArrOperation)settable_).endCalculate(_nodes, staticPostEltContent_.isPost(), _stored, _res, _advStandards, _context, _rendStackCall);
        }
        if (settable_ instanceof RendSettableCallFctOperation) {
            arg_ = ((RendSettableCallFctOperation)settable_).endCalculate(_nodes, staticPostEltContent_.isPost(), _stored, _res, _advStandards, _context, _rendStackCall);
        }
