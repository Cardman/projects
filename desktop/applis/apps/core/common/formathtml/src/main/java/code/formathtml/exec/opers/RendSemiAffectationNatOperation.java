...
public final class RendSemiAffectationNatOperation extends RendSemiAffectationOperation {
    ...
    protected void calculateSpec(IdMap<RendDynOperationNode, ArgumentsPair> _nodes, Configuration _conf, BeanLgNames _advStandards, ContextEl _context, StackCall _stack, RendStackCall _rendStack) {
        ...
        if (converterTo != null) {
            ...
            conv_ = RendAffectationOperation.calculateChSetting(getSettable(),_nodes,_conf,conv_, _advStandards, _context, _stack, _rendStack);
    ...
    private Argument calculateSemiChSetting(IdMap<RendDynOperationNode, ArgumentsPair> _nodes, Configuration _conf, Argument _stored, BeanLgNames _advStandards, ContextEl _context, StackCall _stackCall, RendStackCall _rendStackCall) {
        ...
        if (settable_ instanceof RendStdRefVariableOperation) {
            arg_ = ((RendStdRefVariableOperation)settable_).calculateSemiSetting(_nodes, _conf, getOperatorContent().getOper(), getStaticPostEltContent().isPost(),_stored, getResultClass().getUnwrapObjectNb(), _advStandards, _context, _stackCall, _rendStackCall);
        }
        if (settable_ instanceof RendSettableFieldOperation) {
            arg_ = ((RendSettableFieldOperation)settable_).calculateSemiSetting(_nodes, _conf, getOperatorContent().getOper(), getStaticPostEltContent().isPost(),_stored, getResultClass().getUnwrapObjectNb(), _advStandards, _context, _stackCall, _rendStackCall);
        }
        if (settable_ instanceof RendCustArrOperation) {
            arg_ = ((RendCustArrOperation)settable_).calculateSemiSetting(_nodes, _conf, getOperatorContent().getOper(), getStaticPostEltContent().isPost(),_stored, getResultClass().getUnwrapObjectNb(), _advStandards, _context, _stackCall, _rendStackCall);
        }
        if (settable_ instanceof RendArrOperation) {
            arg_ = ((RendArrOperation)settable_).calculateSemiSetting(_nodes, _conf, getOperatorContent().getOper(), getStaticPostEltContent().isPost(),_stored, getResultClass().getUnwrapObjectNb(), _advStandards, _context, _stackCall, _rendStackCall);
        }
        if (settable_ instanceof RendSettableCallFctOperation) {
            arg_ = ((RendSettableCallFctOperation)settable_).calculateSemiSetting(_nodes, _conf, getOperatorContent().getOper(), getStaticPostEltContent().isPost(),_stored, getResultClass().getUnwrapObjectNb(), _advStandards, _context, _stackCall, _rendStackCall);
        }
