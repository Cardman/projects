...
public final class RenderInitStdsTest extends CommonRender {
    ...
    public void process6Test() {
        ...
        RendStackCall rendStackCall_ = new RendStackCall(InitPhase.NOTHING, contextEl_);
        BeanLgNames standards_ = (BeanLgNames) contextEl_.getStandards();
        CommonRender.getHeaders(new StringMap<String>(), a_);
        String err_ = RendDimensionArrayInstancing.newCustomArrayOrExc(new Ints(), "$int", new Ints(), an_.getContext(), rendStackCall_).getClassName(contextEl_);
        assertEq(standards_.getContent().getCoreNames().getAliasBadSize(),err_);
        err_ = RendDimensionArrayInstancing.newCustomArrayOrExc(new Ints(), "$int", new Ints(-1), an_.getContext(), rendStackCall_).getClassName(contextEl_);
        assertEq(standards_.getContent().getCoreNames().getAliasBadSize(),err_);
