...
//        return transformSpecialChars(_htmlText, possibleEncodes(true, false));
//    }
//
...
//    }
    ...
    public static String encodeHtml(String _htmlText) {
        return encodeHtml(_htmlText,new CustList<EncodedChar>());
//        CustList<EncodedChar> map_ = new CustList<EncodedChar>();
//        format(map_);
...
//        return AbstractEncodingText.encodeCommon(_htmlText, new CustList<EncodedChar>(), length_, incr_);
    }
    public static String encodeHtml(String _htmlText, CustList<EncodedChar> _map) {
        int length_ = _htmlText.length();
        StringBuilder str_ = new StringBuilder();
        int i_ = 0;
        AbstractEncodingText incr_ = new TryIncrEncodingText(i_,str_);
...
//        StringMap<String> enc_ = new StringMap<String>();
//        mergeInto(_input, until_, enc_);
//        CustList<EncodedChar> chs_ = build(enc_);
...
//    }
    ...
    public static CustList<EncodedChar> build(StringMap<String> _enc) {
        ...
        for (EntryCust<String,String> e: _enc.entryList()) {
            out_.add(new EncodedChar(e.getKey(),e.getValue().charAt(0)));
        }
        return out_;
    }
    ...
    public static void mergeInto(String _str, int _from, int _until, StringMap<String> _enc) {
        ...
        int i_ = 0;
        int c_ = relevant_.size();
        while (i_ < c_) {
            if (i_ + 1 < c_) {
                String value_ = relevant_.get(i_+1);
                int ascii_ = (int) NumberUtil.parseLongZero(value_);
                char char_ = (char) ascii_;
                _enc.put("&"+relevant_.get(i_)+";",""+char_);
            }
            i_+=2;
        }
    }
    ...
    public static DocumentResult parseSaxNotNullRowCol(String _xml) {
        return parseSaxNotNullRowCol(_xml,new StringMap<String>());
    }
    public static DocumentResult parseSaxNotNullRowCol(String _xml, StringMap<String> _encoded) {
        DocumentBuilder builder_ = newXmlDocumentBuilder();
        return builder_.parse(_xml,_encoded);
    }
