...
//        SortedPlayers.nextPlayers(joueursRepartitionConnueMemo_, joueursRepartitionInconnue_, nbPlayers);
    ...
    private void iterate(Ints _joueursRepartitionConnue, Ints _joueursRepartitionConnue2, Ints _joueursRepartitionConnueMemo, Ints _joueursRepartitionInconnue, IdMap<Suit, CustList<HandBelote>> _cartesPossibles, IdList<Suit> _toutesCouleurs, IdMap<Suit, CustList<HandBelote>> _cartesCertaines) {
    ...
    private void validatePlayerBelote(IdMap<Suit, CustList<HandBelote>> _cartesPossibles, IdList<Suit> _toutesCouleurs, IdMap<Suit, CustList<HandBelote>> _cartesCertaines, int _joueur) {
        for (Suit couleur_: _toutesCouleurs) {
            CustList<HandBelote> cardSuit_ = _cartesPossibles.getVal(couleur_);
            HandBelote sure_ = _cartesCertaines.getVal(couleur_).get(_joueur);
            for (CardBelote carte_ : cardSuit_.get(
                ...
                if (nombreDApparitionCarte_ == 1) {
                    sure_.removeCardIfPresent(carte_);
                    sure_.ajouter(carte_);
                }
            }
        }
    }

    private void remImposBelote(IdMap<Suit, CustList<HandBelote>> _cartesPossibles, IdList<Suit> _toutesCouleurs, IdMap<Suit, CustList<HandBelote>> _cartesCertaines, int _joueur, int _joueur2) {
        for (Suit couleur_: _toutesCouleurs) {
    ...
    private void remPoss(IdMap<Suit, CustList<HandBelote>> _cartesPossibles, IdMap<Suit, CustList<HandBelote>> _cartesCertaines, int _joueur, int _joueur2, Suit _couleur) {
    ...
    private void remSure(IdMap<Suit, CustList<HandBelote>> _cartesCertaines, int _joueur, int _joueur2, Suit _couleur) {
    ...
    private IdMap<Suit, CustList<HandBelote>> sure(Ints _joueursRepartitionConnue, Ints _joueursRepartitionConnueMemo, IdMap<Suit, CustList<HandBelote>> _cartesPossibles, IdList<Suit> _toutesCouleurs) {
        ...
        for (EntryCust<Suit, CustList<HandBelote>> couleur_:cartesCertaines_.entryList()) {
            for (int joueur_ = IndexConstants.FIRST_INDEX; joueur_ < nbPlayers; joueur_++) {
                couleur_.getValue().add(new HandBelote());
            }
        }
        for (int joueur_ = IndexConstants.FIRST_INDEX; joueur_ < nbPlayers; joueur_++) {
    ...
    void sortSuits(IdMap<Suit, CustList<HandBelote>> _reps, int _player) {
...
//                o_ = Order.TRUMP;
//            }
            GameBeloteCommon.hand(_reps,couleur_, _player).trierUnicolore(true,bid);
        }
    }

    void addToKnown(IdList<Suit> _all,IdMap<Suit,CustList<HandBelote>> _poss,int _player,
    ...
    static void affect(IdList<Suit> _all,IdMap<Suit,CustList<HandBelote>> _from, int _player,IdMap<Suit,CustList<HandBelote>> _to) {
        for (Suit s: _all) {
        ...
        if (couleurDemandee_ == _couleurAtout) {
            for (CardBelote c: progressingTrick) {
                ...
                if (couleurJoueur_ != _couleurAtout) {
                    int p_ = progressingTrick.joueurAyantJoue(c, nbPlayers);
    ...
    private void trickTrump(Suit _couleurAtout, int _next, CustList<HandBelote> _m) {
    ...
    private void trickCardTrump(int _next, CustList<HandBelote> _m, TrickBelote _pli, Suit _couleurDemande, CardBelote _c) {
        int joueur_ = _pli.joueurAyantJoue(_c,nbPlayers);
        if(joueur_ == _next) {
            return;
        }
        Ints joueursAvant_ = _pli.joueursAyantJoueAvant(joueur_, rules.getDealing());
        int forceLoc_ = _c.strength(_couleurDemande, bid);
        int ramasseurVirtuel_ = ramasseurVirtuel(_pli,_couleurDemande,joueur_,joueursAvant_,forceLoc_);
        //joueursAvant non vide
        if(ramasseurVirtuel_ == joueur_) {
            return;
        }
        int max_ = _pli.carteDuJoueur(ramasseurVirtuel_, nbPlayers).strength(_couleurDemande, bid);
        HandBelote cartesExclues_ = excHand(_couleurDemande, max_, _m.get(joueur_));
        _m.get(joueur_).supprimerCartes(cartesExclues_);
    }

    private void trickNormal(Suit _couleurAtout, int _next, CustList<HandBelote> _m) {
        CustList<TrickBelote> plis_ = new CustList<TrickBelote>(tricks.mid(RulesBelote.offset(rules)));
        plis_.add(progressingTrick);
        for(TrickBelote pli_:plis_) {
            Suit couleurDemande_=pli_.couleurDemandee();
    ...
    private void trickCardNormal(Suit _couleurAtout, int _next, CustList<HandBelote> _m, TrickBelote _pli, Suit _couleurDemande, CardBelote _c) {
        ...
        int ramasseurVirtuel_ = ramasseurVirtuel(_pli,_couleurDemande,joueur_,joueursAvant_, 0);
        int max_ = _pli.carteDuJoueur(ramasseurVirtuel_, nbPlayers).strength(_couleurDemande, bid);
    ...
    private void sameTeam(CustList<HandBelote> _m, Suit _couleurDemande, int _joueur, int _max, boolean _defausseSurAtout) {
    ...
    private void usingTrump(CustList<HandBelote> _m, TrickBelote _pli, Suit _couleurDemande, int _joueur, Ints _joueursAvant, int _forceLoc) {
        int ramasseurVirtuel_ = ramasseurVirtuel(_pli, _couleurDemande, _joueur, _joueursAvant, _forceLoc);
    ...
    private int ramasseurVirtuel(TrickBelote _pli, Suit _couleurDemande, int _joueur, Ints _joueursAvant, int _forceLoc) {
        int max_ = 0;
        int ramasseurVirtuel_ = _joueur;
        for(int j: _joueursAvant) {
            CardBelote carte_ = _pli.carteDuJoueur(j,nbPlayers);
            int forceLoc2_ = carte_.strength(_couleurDemande, bid);
    ...
    private HandBelote exc(CustList<HandBelote> _m, Suit _couleurDemande, int _joueur, int _max) {
        return excHand(_couleurDemande, _max, _m.get(_joueur));
    }

    private HandBelote excHand(Suit _couleurDemande, int _max, HandBelote _list) {
    ...
    private void belReb(int _next, CustList<HandBelote> _m) {
        ...
        for(TrickBelote p: tricks.mid(RulesBelote.offset(rules))) {
            for(CardBelote c: p) {
                int joueur_ = p.joueurAyantJoue(c);
    ...
    private void playerTr(Suit _couleurAtout, HandBelote _trumps, HandBelote _cartesJouees, int _next, int _j, HandBelote _h) {
        if (_j == _next) {
            _h.ajouterCartes(_trumps);
            return;
        }
        feedOtherPlayer(_trumps, _cartesJouees, _j, _h, GameBeloteCommonPlaying.cartes(_couleurAtout,bid));
        if(bid.getCouleurDominante()) {
    ...
    private void filterUnder(CustList<HandBelote> _m, Suit _couleurDemande, int _joueur, int _max, HandBelote _cartesExclues) {
    ...
    HandBelote joueCartePoint(Suit _couleur, int _numero,
...
//        cartesJouees_.ajouterCartes(progressingTrick.getCartes());
        ...
        for (TrickBelote pli_ : plis_) {
            HandBelote mainLocale_ = delTr(_numero, pli_, HandBelote.couleurComplete(_couleur,bid));
            retour_.supprimerCartes(mainLocale_);
        }
        return retour_;
    }

    private HandBelote delTr(int _numero, TrickBelote _pli, HandBelote _all) {
    ...
    HandBelote sousCoupeBelote(Suit _couleur, int _numero,
                               HandBelote _atoutsPossibles) {
        HandBelote retour_ = new HandBelote();
        retour_.ajouterCartes(_atoutsPossibles);
//        HandBelote cartesJouees_ = cartesJouees();
//        cartesJouees_.ajouterCartes(progressingTrick.getCartes());
        ...
        for (TrickBelote pli_ : plis_) {
            HandBelote mainLocale_ = delTr(_numero, pli_, GameBeloteCommonPlaying.cartes(_couleur,bid));
            retour_.supprimerCartes(mainLocale_);
        }
        return retour_;
    }

    private boolean foeLeader(int _numero, TrickBelote _pli) {
        int ramasseur_ = _pli.getRamasseur(bid);
        boolean ramasseurAvantNumero_ = false;
        for (int p: _pli.joueursAyantJoueAvant(_numero,rules.getDealing())) {
            if (p == ramasseur_) {
    ...
    private void excludeByDiscard(Suit _couleur, CustList<HandBelote> _m, int _joueur) {
        if (taker != _joueur) {
    ...
    private void currentTrick(Suit _couleur, CustList<HandBelote> _m, int _next) {
        for (int joueur_ = IndexConstants.FIRST_INDEX; joueur_<nbPlayers; joueur_++) {
            if(joueur_ == _next) {
                continue;
            }
    ...
    private void plNormal(Suit _couleur, HandBelote _suitCards, HandBelote _cartesJouees, int _next, int _joueur, HandBelote _h) {
        if(_joueur == _next) {
    ...
    private void feedOtherPlayer(HandBelote _suitCards, HandBelote _cartesJouees, int _joueur, HandBelote _h, HandBelote _allSuitCards) {
...
//            if (!_cartesJouees.contient(carte_) && !_suitCards.contient(carte_) && (lastSeenHand.estVide() || !CardBelote.eq(lastSeenHand.premiereCarte(), carte_) || _joueur == taker))
    ...
    boolean defausseBelote(Suit _couleur, int _joueur,CustList<TrickBelote> _plisFaits) {
    ...
    private boolean doitCouperMaisNePeutPas(int _joueur, Suit _couleur, CustList<TrickBelote> _plisFaits, int _i) {
        ...
        //defausse de joueur
        Ints joueurs_=pli_.joueursAyantJoueAvant(_joueur, rules.getDealing());
        //Le pli n'est pas vide.
        int ramasseurAvant_ = ramasseurVirtuel(pli_,_couleur,pli_.getEntameur(),joueurs_, 0);
    ...
    static boolean neFournitPas(Suit _couleur, int _joueur,CustList<TrickBelote> _plisFaits) {
        int lastIndex_ = _plisFaits.getLastIndex();
        for(int indicePli_ = lastIndex_; indicePli_>= IndexConstants.FIRST_INDEX; indicePli_--) {
