...
public final class TricksHandsBelote {
    ...
    public void restituerMains(DisplayingBelote _displaying, int _numeroPli, RulesBelote _rules) {
    ...
    public void restituerMains(DisplayingBelote _displaying, int _numeroPli, int _numeroCarte, RulesBelote _rules) {
        init(_numeroPli,_rules);
        int key_ = 1;
        for(TrickBelote pli_:tricks.mid(RulesBelote.offset(_rules))) {
            if(key_ > _numeroPli) {
    ...
    private void init(int _numeroPli, RulesBelote _rules) {
    ...
    private static void restoreTakerHand(DealBelote _deal, int _preneur, CustList<TrickBelote> _tricks) {
    ...
    public static void restoreTakerHand(DealBelote _deal, int _preneur, TrickBelote _possibleFirst) {
    ...
    private void previousTrick(TrickBelote _pli, RulesBelote _rules) {
        int nombreJoueurs_ = _rules.getDealing().getId().getNombreJoueurs();
        for (int p: _pli.playersHavingPlayed(nombreJoueurs_)) {
            getDistribution().jouer(p, _pli.carteDuJoueur(p,nombreJoueurs_));
        }
    }

    private void currentTrick(int _numeroCarte, TrickBelote _pli, RulesBelote _rules) {
        int nombreJoueurs_ = _rules.getDealing().getId().getNombreJoueurs();
        int indice_ = 0;
        for (int p: _pli.playersHavingPlayed(nombreJoueurs_)) {
    ...
    public static void endRestore(CustList<HandBelote> _hands, int _preneur, CustList<TrickBelote> _tricks, RulesBelote _rules) {
        TrickBelote possibleFirst_ = GameBelote.discardedCards(_tricks, new TrickBelote());
        endRestore(_hands, _preneur, possibleFirst_, _rules);
    }

    public static void endRestore(CustList<HandBelote> _hands, int _preneur, TrickBelote _possibleFirst, RulesBelote _rules) {
