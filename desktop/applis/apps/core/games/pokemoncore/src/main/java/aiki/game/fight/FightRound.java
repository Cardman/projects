...
final class FightRound {
    ...
    static void initRound(Fight _fight) {
        for(int e: _fight.getTeams().getKeys()){
    ...
    private static void spendPowerPoint(Difficulty _diff, DataBase _import, Fighter _creature, String _attaqueLanceur) {
        if (_creature.spendPowerPoint(_attaqueLanceur, _import)) {
            _creature.usePowerPointsByMove(_diff, _attaqueLanceur,1);
        }
    }
...
//        }
        ...
        boost_ = FightStatistic.rateBoost(NumberUtil.min(cran_, maxBoost_), _import);
        def_.multiplyBy(boost_);
        StringMap<String> varLocs_ = new StringMap<String>();
        varLocs_.put(_import.prefixAttack(), att_.toNumberString());
        varLocs_.put(_import.prefixDefense(), def_.toNumberString());
        varLocs_.put(_import.prefixLanceurNiveau(), Long.toString(creature_.getLevel()));
        varLocs_.put(_import.prefixPower(), _puissance.toNumberString());
    ...
    static TeamPositionList takers(Fight _fight, TeamPosition _fighter, DataBase _data) {
        ...
        for (TeamPosition t: FightOrder.frontFighters(_fight)) {
            ...
            int pos_ = _fight.getFighter(_fighter).getGroundPlace();
    ...
    static TeamPositionList selectTargetHavingToPlayAfterThrower(Fight _fight,TeamPosition _lanceur,DataBase _import){
        ...
        Team equipeCbt_=_fight.getTeams().getVal(targetCoords_.getTeam());
        Ints ciblesEquipe_=equipeCbt_.fightersAtCurrentPlace(targetCoords_);
        ...
        for(int c:ciblesEquipe_){
            TeamPosition teamPos_ = new TeamPosition(targetCoords_.getTeam(),c);
    ...
    static void roundThrowerHealing(Fight _fight,TeamPosition _lanceur, Difficulty _diff,DataBase _import){
        ...
        if (ac_.isTeam()) {
            ...
            if (_fight.getState() == FightState.SWITCH_WHILE_KO_USER && NumberUtil.eq(_lanceur.getTeam(), Fight.CST_PLAYER)) {
                ...
                for (int m = IndexConstants.FIRST_INDEX; m < mult_; m++) {
                ...
                int pl_ = _fight.getPlayerMaxNumberFrontFighters() - allyPk_.getGroundPlace();
    ...
    static void roundThrowerSwitch(Fight _fight,TeamPosition _lanceur,Difficulty _diff,DataBase _import){
        Fighter creature_=_fight.getFighter(_lanceur);
        int remplacant_=creature_.getSubstistute();
        TeamPosition envoye_=new TeamPosition(_lanceur.getTeam(),remplacant_);
        Fighter partenaire_=_fight.getFighter(envoye_);
        int placeTerrain_=creature_.getGroundPlace();
        AnimationSwitch animation_;
        animation_ = new AnimationSwitch();
        animation_.setIndex(_fight.getEffects().size());
        int placeTerrainPourRemplacement_=creature_.getGroundPlaceSubst();
    ...
    static Rate calculateCatchingRate(Fight _fight, FighterPosition _creatureUt, String _nomBall, boolean _dejaCapture, Difficulty _diff, DataBase _import, Fighter _creatureSauvage){
        if(_creatureSauvage.estKo()){
            return DataBase.determinatedRate();
        }
        Ball ball_=(Ball)_import.getItem(_nomBall);
        //CustList<Integer> cbts_=_fight.getUserTeam().fightersAtCurrentPlace((short) CustList.FIRST_INDEX);
        //Fighter creatureUt_=_fight.getUserTeam().refPartMembres(cbts_.first());
        StringMap<String> variables_=calculateCatchingVariables(_fight, _creatureUt, _dejaCapture, _import, _creatureSauvage);
...
//        variables_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.PK_SAUVAGE_PIERRES_EVOS),pierresEvo_.join(NumericString.SEPARATOR_SET));
        ...
        for(String c: _creatureSauvage.getStatusSet()){
            if(_creatureSauvage.getStatusNbRoundShort(c) == 0){
                continue;
            }
        ...
        if (!_diff.getAllowCatchingKo() && Rate.lowerEq(_creatureSauvage.getRemainingHp(), _import.getMinHp())) {
            return DataBase.determinatedRate();
        }
        variables_.clear();
        variables_.put(_import.prefixBaseCaptPk(), Long.toString(fPk_.getCatchingRate()));
        variables_.put(_import.prefixRateBallStatus(), taux_.toNumberString());
        variables_.put(_import.prefixFoePkMaxHp(), _creatureSauvage.pvMax().toNumberString());
        variables_.put(_import.prefixFoePkRemoteHp(), _creatureSauvage.getRemainingHp().toNumberString());
...
//        CustList<Integer> cbts_=_fight.getUserTeam().fightersAtCurrentPlace((short) CustList.FIRST_INDEX);
        ...
        if(_dejaCapture){
            variables_.put(_import.prefixDejaCapture(),Fight.ONE);
        }else{
            variables_.put(_import.prefixDejaCapture(),Fight.ZERO);
        }
    ...
    static MonteCarloNumber calculateFleeingRate(Fight _fight,Difficulty _diff,DataBase _import){
        MonteCarloNumber mcn_ = new MonteCarloNumber();
        if(!_diff.getStillPossibleFlee()){
            for (EntryCust<Integer,Fighter> e: _fight.getFoeTeam().getMembers().entryList()) {
                if (!e.getValue().estKo() && FightOrder.notCaught(_fight,Fight.toFoeFighter(e.getKey()))) {
                    StringMap<String> variables_= calculateFleeingVariable(_fight, _import, e.getValue());
                    String flee_ = _import.getRateFleeing();
                    mcn_.addQuickEvent(_import.evaluateNumericable(flee_, variables_, Rate.one()),LgInt.one());
                }
            }
        }
        if (mcn_.isZero()) {
            mcn_.addQuickEvent(DataBase.determinatedRate(),LgInt.one());
        }
        return mcn_;
//        if(_diff.getStillPossibleFlee()){
//            return DataBase.determinatedRate();
//        }
        ...
        //CustList<Integer> cbts_=_fight.getUserTeam().fightersAtCurrentPlace((short) CustList.FIRST_INDEX);
        //Fighter creatureUt_=_fight.getUserTeam().refPartMembres(cbts_.first());
//        StringMap<String> variables_= calculateFleeingVariable(_fight, _import, _fight.wildPokemon());
//        variables_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LIEU_COMBAT),_fight.getEnvType().name());
...
//        CustList<Integer> cbts_=_fight.getUserTeam().fightersAtCurrentPlace((short) CustList.FIRST_INDEX);
        ...
        StringMap<String> variables_=new StringMap<String>();
        variables_.put(_import.prefixNbFlees(),Long.toString(_fight.getNbFleeAttempt()));
    ...
    private static StringMap<String> varsCatchFlee(Fight _fight, DataBase _import, Fighter _creatureUt, Fighter _creatureSauvage, StringMap<String> _variables, TeamPosition _cbt) {
        _variables.put(_import.prefixLieuCombat(), _fight.getEnvType().getEnvName());
        _variables.put(_import.prefixTempsTour(), _fight.getNbRounds().toNumberString());
        _variables.put(_import.prefixMasseMoyennePk(), _import.getAvgWeight().toNumberString());
        _variables.put(_import.prefixPkUtGenre(), _creatureUt.getGender().getGenderName());
        _variables.put(_import.prefixPkUtMasse(), _creatureUt.getWeight().toNumberString());
        _variables.put(_import.prefixPkUtNiveau(),Long.toString(_creatureUt.getLevel()));
        _variables.put(_import.prefixPkUtVitesse(),FightOrder.speed(_fight, _cbt, _import).toNumberString());
        _variables.put(_import.prefixPkSauvageGenre(), _creatureSauvage.getGender().getGenderName());
        _variables.put(_import.prefixPkSauvageMasse(), _creatureSauvage.getWeight().toNumberString());
        _variables.put(_import.prefixPkSauvageNiveau(),Long.toString(_creatureSauvage.getLevel()));
        _variables.put(_import.prefixPkSauvageVitesse(),FightOrder.speed(_fight,Fight.toFoeFighter(IndexConstants.FIRST_INDEX), _import).toNumberString());
        PokemonData fPkSauvage_= _creatureSauvage.fichePokemon(_import);
        _variables.put(_import.prefixPkSauvageTypesBase(), StringUtil.join(fPkSauvage_.getTypes(), _import.getSepartorSetChar()));
        _variables.put(_import.prefixPkSauvagePierresEvos(), StringUtil.join(pierresEvo(fPkSauvage_), _import.getSepartorSetChar()));
        PokemonData fPkUt_= _creatureUt.fichePokemon(_import);
        _variables.put(_import.prefixPkUtTypesBase(), StringUtil.join(fPkUt_.getTypes(), _import.getSepartorSetChar()));
        _variables.put(_import.prefixPkUtPierresEvos(), StringUtil.join(pierresEvo(fPkUt_), _import.getSepartorSetChar()));
        return _variables;
    }
