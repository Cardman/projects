...
//        DIALOG.messages = ExtractFromFiles.getMessagesFromLocaleClass(FileConst.FOLDER_MESSAGES_GUI, Constants.getLanguage(), DIALOG.getClass());
    ...
    private void setDialogue(String _menu, WindowCardsInt _fenetre) {
        ...
        if(StringUtil.quickEq(menu, WindowCards.CST_LAUNCHING)) {
            ...
            mess_.put(GameEnum.NONE, messSoft_.getVal(MessagesGuiCards.LAUNCHING));
...
//            panneau_.add(liste);
            ...
            saveHomeFolder = getCompoFactory().newCustCheckBox(messSoft_.getVal(MessagesGuiCards.SELECT_HOME_PATH));
...
//            JSlider barre_=null;
            ...
            int minValeur_ = 300;
            int maxValeur_ = 2000;
            String sentence_ = messSoft_.getVal(MessagesGuiCards.WAITING_SENTENCE);
            String prefix_ = messSoft_.getVal(MessagesGuiCards.WAITING_BIDDING);
            String values_ = StringUtil.simpleNumberFormat(messSoft_.getVal(MessagesGuiCards.WAITING_VALUES), minValeur_, maxValeur_, valeur_);
            ...
            delayWaitBids.addChangeListener(new ListenerChangeSlide(delayWaitBids,label_,MessagesGuiCards.WAITING_BIDDING, messSoft_, sentence_));
            panneau_.add(delayWaitBids);
//            indiceInfo_++;
            valeur_=parametres.getDelaiAttenteCartes();
            prefix_ = messSoft_.getVal(MessagesGuiCards.WAITING_PLAYED_CARD);
            values_ = StringUtil.simpleNumberFormat(messSoft_.getVal(MessagesGuiCards.WAITING_VALUES),minValeur_, maxValeur_, valeur_);
            ...
            delayWaitCards.addChangeListener(new ListenerChangeSlide(delayWaitCards,label_,MessagesGuiCards.WAITING_PLAYED_CARD, messSoft_, sentence_));
            panneau_.add(delayWaitCards);
//            indiceInfo_++;
            valeur_=parametres.getDelaiAttentePlis();
            minValeur_=500;
            maxValeur_=3000;
            prefix_ = messSoft_.getVal(MessagesGuiCards.WAITING_TRICK);
            values_ = StringUtil.simpleNumberFormat(messSoft_.getVal(MessagesGuiCards.WAITING_VALUES),minValeur_, maxValeur_, valeur_);
            label_ = getCompoFactory().newPlainLabel(StringUtil.simpleStringsFormat(sentence_, prefix_, values_));
            panneau_.add(label_);
            delayWaitTricks=getCompoFactory().newAbsSlider(minValeur_,maxValeur_);
            delayWaitTricks.setValue(valeur_);
            delayWaitTricks.addChangeListener(new ListenerChangeSlide(delayWaitTricks,label_,MessagesGuiCards.WAITING_TRICK, messSoft_, sentence_));
            panneau_.add(delayWaitTricks);
//            indiceInfo_++;
            waitTrickClick =getCompoFactory().newCustCheckBox(messSoft_.getVal(MessagesGuiCards.CLICK_FOR_PLAYING_TRICK));
            ...
            AbsPanel panneau_=_fenetre.getCompoFactory().newGrid(0,1);
            clickCard=getCompoFactory().newCustCheckBox(messSoft_.getVal(MessagesGuiCards.CLICK_FOR_PLAYING_CARD));
            clickCard.setSelected(parametres.getJeuCarteClic());
            panneau_.add(clickCard);
            container_.add(panneau_,GuiConstants.BORDER_LAYOUT_CENTER);
        }
        validate = getCompoFactory().newPlainButton(messSoft_.getVal(MessagesGuiCards.VALIDATE));
