...
public class CardTarotCellRenderer implements AbsCustCellRenderGene<CardTarot> {
    ...
    public static void updateImage(WindowCardsInt _win, CouleurValeur _id, boolean _sel, AbstractImage _img) {
        if(!_sel) {
            _img.drawImage(ConverterGraphicBufferedImage.decodeToImage(_win.getImageFactory(), _win.getImagesMiniDef().getVal(_win.getFrames().getLanguage()).getVal(""+ _id.nb())),0,0);
        } else {
            _img.drawImage(ConverterGraphicBufferedImage.decodeToImage(_win.getImageFactory(), _win.getImagesMiniSel().getVal(_win.getFrames().getLanguage()).getVal(""+ _id.nb())),0,0);
        }
    }
