...
public final class TargetsPanel {
...
//        teamPl_.putAllMap(_facade.getUnionFrontTeam());
        ...
        for (ChosableTargetName k: chosablePl_) {
            ...
            target_.set(_facade,_battle, k.getName());
            boolean match_ = k.getChosable() == BoolVal.TRUE;
            if (match_) {
                target_.addMouseListener(new PkNonModalEvent(_battle.getWindow().getModal()),new SelectPlayerTarget(_battle, k.getKey(), target_));
            }
        ...
        for (ChosableTargetName k: chosableFoe_) {
            ...
            boolean match_ = k.getChosable() == BoolVal.TRUE;
            if (match_) {
                target_.addMouseListener(new PkNonModalEvent(_battle.getWindow().getModal()),new SelectFoeTarget(_battle, k.getKey(), target_));
            }
            target_.setSelectable(match_);
            AbsMetaLabelPk.paintPk(_battle.getWindow().getImageFactory(), target_);
    ...
    public void repaintLabel(MiniTargetLabel _index, Battle _battle) {
