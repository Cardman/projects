...
//        unitMessages = ResourcesMessagesUtil.getMessagesFromContent(loadedResourcesMessages_);
        StringMap<String> spe_ = ProgTestBar.valExecOptionsMain(getFrames().currentLg());
        StringMap<String> mesUnit_ = ProgTestBar.valExecOptionsTable(getFrames().currentLg());
        setTitle(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_TITLE));
        setJMenuBar(getCompoFactory().newMenuBar());
        menu = getCompoFactory().newMenu(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_FILE));
        open = getCompoFactory().newMenuItem(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_OPEN));
        ...
        logErr = getCompoFactory().newCheckBoxMenuItem(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_STATUS));
        logErr.addActionListener(new LogErrEvent(this));
        menu.addMenuItem(logErr);
        memory = getCompoFactory().newCheckBoxMenuItem(spe_.getVal(ProgTestBar.EXEC_OPTIONS_MAIN_MEMORY));
        menu.addMenuItem(memory);
        menu.addMenuItem(getCompoFactory().newSep());
        simpleFrame = getCompoFactory().newMenuItem(spe_.getVal(ProgTestBar.EXEC_OPTIONS_MAIN_ARCHIVE));
        simpleFrame.addActionListener(new SimpleFilesFrameOpen(this));
        menu.addMenuItem(simpleFrame);
        menu.addMenuItem(getCompoFactory().newSep());
        stop = getCompoFactory().newMenuItem(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_STOP));
        ...
        content = getCompoFactory().newPlainLabel(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_CONFIGURATION));
        form.add(content);
        ...
        form.add(scr_);
        launch = getCompoFactory().newPlainButton(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_LAUNCH));
        ...
        doneTests = getCompoFactory().newPlainLabel(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_TESTS));
        ...
        method = getCompoFactory().newPlainLabel(mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_METHOD));
        ...
        cols_[0] = mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_NUMBER);
        cols_[1] = mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_METHOD);
        cols_[2] = mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_PARAMS);
        cols_[3] = mesUnit_.getVal(ProgTestBar.EXEC_OPTIONS_TABLE_SUCCESS);
