...
public final class CustThreadActions extends AbstractThreadActions {
    ...
    private CustThreadActions(){}

    public static CustThreadActions inst(RenderedPage _page, BeanCustLgNames _lgNames, String _lgCode, String _anchor, String _fileName, StringMap<String> _fileNames) {
        CustThreadActions t_ = new CustThreadActions();
        t_.setPage(_page);
        t_.getPage().start();
        t_.stds = _lgNames;
        t_.lgCode = _lgCode;
    ...
    public void run() {
        if (usedFirstUrl) {
            ...
            if (!getPage().getNavigation().isError()) {
                ...
                Configuration conf_ = getPage().getNavigation().getSession();
                if (fileNames != null) {
