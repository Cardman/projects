package cards.network.threads;

import cards.gui.containers.ContainerMultiTarot;
import cards.network.tarot.unlock.AllowPlayingTarot;
import code.gui.initialize.AbstractSocket;
import code.network.WindowNetWork;
import code.util.CustList;

public final class ClientActLoopCardsAllowPlayingTarot implements IntClientActLoopCards {
    @Override
    public void loop(WindowNetWork _window, CustList<String> _parts, AbstractSocket _socket) {
        AllowPlayingTarot ch_ = Net.importAllowPlayingTarot(_parts);
        ContainerMultiTarot containerTarot_ = (ContainerMultiTarot) _window.getNetg().getContainerGame();
        containerTarot_.canPlayTarot(ch_);
    }
}
