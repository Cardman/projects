...
//            }
        ...
        if (_action instanceof CallAfterDiscardTarot && _action.getActionType() == PlayerActionGameType.SIMPLE) {
            processCallingTarot((CallAfterDiscardTarot) _action, _instance,_fct,_common);
            return;
        }
        if (_action instanceof DiscardedCardTarot) {
            GameTarot game_ = Net.getGames(_instance).partieTarot();
            DiscardedCardTarot discarded_ = (DiscardedCardTarot) _action;
...
//                    return;
//                }
//                game_.ecart(_instance.getIa().getTarot());
//                game_.gererChienInconnu();
//                AfterAnimationBidTarot.possibleSlam(game_, _instance.getIa());
...
//            processDiscardingTrumps((SeenDiscardedTrumps) _action, _instance,_fct,_common);
//            return;
//        }
...
//        }
        ...
        if (_action instanceof CallAfterDiscardTarot) {
            ContainerSingleTarot.playerCall(Net.getGames(_instance).partieTarot(),((CallAfterDiscardTarot)_action).getCalledCards(),_instance.getIa());
        }
        if (actionType_ == PlayerActionGameType.VALIDATE_DOG || actionType_ == PlayerActionGameType.SLAM) {
...
//                game_.setPliEnCours(true);
        ...
        if (_action instanceof PlayingCardTarot && !((PlayingCardTarot)_action).isRefreshing()) {
        ...
        if (_action instanceof PlayingCardTarot) {
            ...
            p_.setPlayedCard(((PlayingCardTarot)_action).getPlayedCard());
            p_.setChoosenHandful(((PlayingCardTarot)_action).getChoosenHandful());
            p_.setHandful(((PlayingCardTarot)_action).getHandful());
            p_.setMiseres(((PlayingCardTarot)_action).getMiseres());
            p_.setCalledCard(((PlayingCardTarot)_action).isCalledCard());
...
//    }
    ...
    private static void processCallingTarot(CallAfterDiscardTarot _readObject, Net _instance, AbstractThreadFactory _fct, NetCommon _common) {
...
//        return;
    ...
    private static void update(byte _player, GameBelote _game, DiscardPhaseBelote _dog) {
        if (_player == _game.getPreneur()) {
...
//            }
        ...
        if (_action instanceof DiscardedCardBelote) {
             GameBelote game_ = Net.getGames(_instance).partieBelote();
             DiscardedCardBelote discarded_ = (DiscardedCardBelote) _action;
             if (game_.getPliEnCours().getCartes().contient(discarded_.getCard())) {
...
//            return;
//        }
        AfterAnimationBidTarot.intelligenceArtificielleAppel(_game, _instance.getIa());
        ContainerSingleTarot.ecart(_game,_instance.getIa());
//        if (!_game.getRegles().getDiscardAfterCall()) {
//            if (_game.getContrat().getJeuChien() == PlayingDog.WITH) {
//                _game.appelApresEcart(_instance.getIa().getTarot());
...
//                }
        ...
        if (_action instanceof PlayingCardBelote && !((PlayingCardBelote)_action).isRefreshing()) {
...
//                error_.setReason(Games.autoriseBelote(game_, _common.getProgramInfos().getTranslations().getMapping().getVal(info_.getLocale())));
//                Net.sendObject(Net.getSocketByPlace(info_.getPlace(), _common), error_);
//                return;
//            }
...
//            ref_.setLocale("");
        ...
        if (_action instanceof PlayingCardBelote) {
            ...
            p_.setTakerIndex(game_.getPreneur());
            p_.setPlace(_action.getPlace());
            p_.setPlayedCard(((PlayingCardBelote)_action).getPlayedCard());
            p_.setDeclaringBeloteRebelote(((PlayingCardBelote)_action).isDeclaringBeloteRebelote());
            p_.setDeclaring(((PlayingCardBelote)_action).isDeclaring());
            p_.setDeclare(((PlayingCardBelote)_action).getDeclare());
            //p_.setLocale(Constants.getDefaultLanguage());
//            p_.setLocale("");
            Net.initAllReceived(_instance, _common);
            for (byte p: Net.activePlayers(_instance, _common)) {
...
//                        }
        ...
        if (_action instanceof DiscardedCardsPresident) {
            Bytes pl_ = Net.activePlayers(_instance, _common);
            DiscardedCardsPresident dis_ = (DiscardedCardsPresident) _action;
        ...
        if (_action instanceof PlayingCardPresident && !((PlayingCardPresident)_action).isRefreshing()) {
...
//                cardDto_.setLocale("");
        ...
        if (_action instanceof PlayingCardPresident) {
            ...
            GamePresident game_ = Net.getGames(_instance).partiePresident();
            cardDto_.setPlayedHand(((PlayingCardPresident)_action).getPlayedHand());
