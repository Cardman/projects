...
public final class PrimitiveTypeUtilAdv {
    ...
    public static Struct newCustomArray(String _className, Numbers<Integer> _dims, ContextEl _cont) {
        ...
        Struct output_ = new ArrayStruct(instanceGl_, PrimitiveTypeUtilAdv.getPrettyArrayType(_className, _dims.size()));
        ...
        for (int i : _dims) {
            ...
            String formattedClass_ = PrimitiveTypeUtilAdv.getPrettyArrayType(_className, glDim_);
            for (Numbers<Integer> k: dims_.getAllIndexes()) {
    ...
    static boolean isArrayAssignable(String _arrArg, String _arrParam, ContextEl _classes) {
        ...
        LgNames stds_ = _classes.getStandards();
        DimComp dArg_ = PrimitiveTypeUtilAdv.getQuickComponentBaseType(_arrArg);
        String a_ = dArg_.getComponent();
        DimComp dPar_ = PrimitiveTypeUtilAdv.getQuickComponentBaseType(_arrParam);
    ...
    public static boolean canBeUseAsArgument(String _param, String _arg, ContextEl _context) {
        ...
        ClassArgumentMatchingAdv param_ = new ClassArgumentMatchingAdv(_param);
        if (_arg == null) {
        ...
        ClassArgumentMatchingAdv arg_ = new ClassArgumentMatchingAdv(_arg);
        ...
        if (paramComp_.getDim() == argComp_.getDim()) {
            param_ = new ClassArgumentMatchingAdv(paramComp_.getComponent());
            arg_ = new ClassArgumentMatchingAdv(argComp_.getComponent());
            clArg_ = new ClassArgumentMatchingAdv(argComp_.getComponent()).getClazz();
        ...
        if (!array_) {
            String typeNameArg_ = PrimitiveTypeUtilAdv.toPrimitive(arg_, true).getName();
            if (StringList.quickEq(typeNameArg_, PrimitiveTypeUtilAdv.PRIM_BOOLEAN)) {
                String typeNameParam_ = PrimitiveTypeUtilAdv.toPrimitive(param_, true).getName();
                if (!StringList.quickEq(typeNameParam_, PrimitiveTypeUtilAdv.PRIM_BOOLEAN)) {
                    return false;
                }
                return true;
            }
            ClassArgumentMatching clMatch_ = PrimitiveTypeUtilAdv.toPrimitive(arg_, true);
            if (clMatch_.isPrimitive(_context)) {
                if (arg_.isPrimitive(_context)) {
                    CustList<ClassArgumentMatching> gt_ = PrimitiveTypeUtilAdv.getOrdersGreaterEqThan(clMatch_);
                    if (isPureNumberClass(clMatch_) && StringList.quickEq(_param, Number.class.getName())) {
                        return true;
                    }
                    ClassArgumentMatching prim_ = PrimitiveTypeUtilAdv.toPrimitive(param_, true);
                ...
                CustList<ClassArgumentMatching> gt_ = PrimitiveTypeUtilAdv.getOrdersGreaterEqThan(clMatch_);
                ClassArgumentMatchingAdv prim_ = param_;
                boolean contained_ = false;
                for (ClassArgumentMatching c: gt_) {
                    if (StringList.quickEq(c.getName(), prim_.getName())) {
                        contained_ = true;
    ...
    private static ClassArgumentMatching toPrimitive(
            ClassArgumentMatchingAdv _arg, boolean _id) {
    ...
    static AssignableFrom isAssignableFromCust(String _param,String _arg, ContextEl _classes) {
        ...
        if (classes_ != null) {
            ...
            DimComp dPar_ = PrimitiveTypeUtilAdv.getQuickComponentBaseType(_param);
            String p_ = dPar_.getComponent();
            RootBlock clParBl_ = classes_.getClassBody(p_);
            DimComp dArg_ = PrimitiveTypeUtilAdv.getQuickComponentBaseType(_arg);
            String a_ = dArg_.getComponent();
            RootBlock clArgBl_ = classes_.getClassBody(a_);
            if (clArgBl_ != null) {
