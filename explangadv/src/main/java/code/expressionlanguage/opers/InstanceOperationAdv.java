...
public class InstanceOperationAdv extends InvokingOperationAdv {
    ...
    void analyzeCtor(CustList<OperationNode> _nodes, ContextEl _conf, String _fieldName, String _op, String _realClassName, CustList<ClassArgumentMatching> _firstArgs, boolean _intern) {
        ...
        if (PrimitiveTypeUtilAdv.isPrimitive(realClassName_, _conf)) {
            throw new PrimitiveTypeException(StringList.concat(realClassName_,RETURN_LINE,_conf.joinPages()));
        }
        Class<?> cl_;
        try {
            cl_ = PrimitiveTypeUtilAdv.getSingleNativeClass(realClassName_);
...
//        }
        ...
        ClassArgumentMatchingAdv arg_ = new ClassArgumentMatchingAdv(realClassName_);
...
//                    setRelativeOffsetPossibleLastPage(o.getIndexInEl()+off_, _conf);
            ...
            if (elts_) {
                if (cust_) {
                    ...
                    dims_.add(nbCh_);
                    Struct str_ = PrimitiveTypeUtilAdv.newCustomArray(realClassName_, dims_, _conf);
                ...
                Struct strArr_ = new StdStruct(array_, PrimitiveTypeUtilAdv.getPrettyArrayType(realClassName_));
                for (int i = CustList.FIRST_INDEX; i < nbCh_; i++) {
                    Argument chArg_ = _arguments.get(i);
                ...
                a_.setStruct(PrimitiveTypeUtilAdv.newCustomArray(realClassName_, dims_, _conf));
                return ArgumentCall.newArgument(a_);
            } else {
                Object o_ = newClassicArray(_conf, instanceClassName_, realClassName_, args_);
                a_.setStruct(new StdStruct(o_, PrimitiveTypeUtilAdv.getPrettyArrayType(realClassName_, args_.length)));
                return ArgumentCall.newArgument(a_);
            }
        }
...
//        }
        ...
        for (String c: constId.getParametersTypes()) {
            ...
            if (j_ + 1 == constId.getParametersTypes().size() && constId.isVararg()) {
                class_ = PrimitiveTypeUtilAdv.getPrettyArrayType(class_);
            }
            params_.add(class_);
            j_++;
        }
        ...
        for (Argument a: _arguments) {
            if (i_ < params_.size()) {
                ...
                if (PrimitiveTypeUtilAdv.primitiveTypeNullObject(params_.get(i_), str_, _conf)) {
                    String null_;
                    null_ = stds_.getAliasNullPe();
                ...
                if (str_ instanceof NumberStruct || str_ instanceof CharStruct) {
                    ...
                    a.setStruct(PrimitiveTypeUtilAdv.convertObject(clArg_, str_, _conf));
                }
            }
            i_++;
        }
    ...
    static Object newClassicArray(ContextEl _conf, String _instanceClassName, String _realClassName,int... _args) {
        Class<?> cl_;
        try {
            if (PrimitiveTypeUtilAdv.isPrimitive(_instanceClassName)) {
                cl_ = PrimitiveTypeUtilAdv.getPrimitiveClass(_instanceClassName);
            } else {
                cl_ = PrimitiveTypeUtilAdv.getSingleNativeClass(_instanceClassName);
            }
