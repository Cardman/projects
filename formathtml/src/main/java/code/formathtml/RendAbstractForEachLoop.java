...
public abstract class RendAbstractForEachLoop extends RendParentBlock implements RendLoop, RendReducableOperations {
    ...
    RendAbstractForEachLoop(Configuration _importingPage,
...
//                    cast_.setFileName(getFile().getFileName());
    ...
    public abstract StringList getInferredIterable(StringList _types,Configuration _cont);
...
//            cast_.setFileName(getFile().getFileName());
    ...
    public abstract String getIteratorVar(Configuration _an);

    public abstract String getHasNextVar(Configuration _an);

    public abstract String getNextVar(Configuration _an);
    public abstract CustList<RendDynOperationNode> getEqIterator(Configuration _an);
    public abstract CustList<RendDynOperationNode> getEqHasNext(Configuration _an);
    public abstract CustList<RendDynOperationNode> getEqNext(Configuration _an);
    ...
    public void processEl(Configuration _cont) {
        ...
        if (el_.getResultClass().isArray()) {
            ...
            Argument arg_ = ElRenderUtil.calculateReuse(getEqIterator(_cont),_cont);
            if (_cont.getContext().hasExceptionOrFailInit()) {
                return;
            }
...
//        ip_.setGlobalOffset(expressionOffset);
    ...
    private Boolean iteratorHasNext(Configuration _conf) {
        ...
        Argument arg_ = ElRenderUtil.calculateReuse(getEqHasNext(_conf),_conf);
        if (_conf.getContext().hasExceptionOrFailInit()) {
            return null;
        }
...
//        abs_.setGlobalOffset(variableNameOffset);
        ...
        if (!el_.getResultClass().isArray()) {
            ...
            arg_ = ElRenderUtil.calculateReuse(getEqNext(_conf),_conf);
        } else {
