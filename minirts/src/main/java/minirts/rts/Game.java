...
//            Polygon pThis_ = new Polygon(xThis_, yThis_, 4);
    ...
    public void setRectangle(CustPoint _first, CustPoint _last) {
        int xLeft_ = Math.min(_first.getX(), _last.getX());
        int xRight_ = Math.max(_first.getX(), _last.getX());
        int yTop_ = Math.min(_first.getY(), _last.getY());
        int yBottom_ = Math.max(_first.getY(), _last.getY());
