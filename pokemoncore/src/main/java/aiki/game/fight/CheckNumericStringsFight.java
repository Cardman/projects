...
public final class CheckNumericStringsFight {
    ...
    public static void validateNumericBooleanStrings(DataBase _data, boolean _validateLevel) {
        ...
        Player user_ = new Player(DataBase.EMPTY_STRING,null,diff_,false,_data);
        ...
        foePokemon_.setMoves(new StringList(_data.getMoves().getKeys().first()));
        ...
        variablesDiff_ = FightValues.calculateValues(fight_, userFighter_, foeFighter_, _data);
        variablesFighter_ = FightValues.calculateValuesFighter(fight_, userFighter_, _data);
        variablesSame_ = FightValues.calculateValues(fight_, userFighter_, userFighter_, _data);
        StringMap<String> boolVarsNotSending_;
        StringMap<String> boolVarsDiffNotSending_;
        StringMap<String> boolVarsSending_;
        StringMap<String> varsSending_ = FightValues.calculateSendingVariables(fight_,userFighter_, _data);
        boolVarsNotSending_ = calculateBooleanValuesForValidation(fight_,userFighter_, userFighter_, false, _data);
        boolVarsDiffNotSending_ = calculateBooleanValuesForValidation(fight_,userFighter_, foeFighter_, false, _data);
        boolVarsSending_ = calculateBooleanValuesForValidation(fight_,userFighter_, userFighter_, true, _data);
        boolVarsSending_.putAllMap(varsSending_);
        checkMovesStr(_data, diff_, fight_, userFighter_,
                foeFighter_, variablesDiff_, variablesSame_,
                boolVarsNotSending_, boolVarsDiffNotSending_, boolVarsSending_,
                varsSending_);
        checkAbilitiesStr(_data, diff_, fight_, userFighter_,
                foeFighter_, variablesDiff_, variablesFighter_, variablesSame_,
        ...
        checkItemsStr(_data, diff_, fight_, userFighter_,
                foeFighter_, variablesDiff_, variablesFighter_, variablesSame_,
                boolVarsNotSending_, boolVarsDiffNotSending_);
    ...
    private static void checkDamageStr(DataBase _data) {
        ...
        varLocs_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTACK), DataBase.defRateProduct().toNumberString());
        varLocs_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.DEFENSE), DataBase.defRateProduct().toNumberString());
        varLocs_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LANCEUR_NIVEAU), Integer.toString(_data.getMinLevel()));
        varLocs_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.POWER), DataBase.getDefaultPower().toNumberString());
        ...
        for (EffectCombo e: _data.getCombos().getEffects().values()) {
            for (EffectEndRound effEndRound_: e.getEffectEndRound()) {
                ...
                checkBoolStrings(_data,_varsDiff,
                        _boolVarsDiffNotSending, effEndRound_.getFailEndRound());
                checkBoolStrings(_data,_varsSame,
                        _boolVarsNotSending, effEndRound_.getFailEndRound());
            }
        }
    }
    ...
    private static void checkStatusStr(DataBase _data,
        ...
        for (Status s: _data.getStatus().values()) {
            for (EffectEndRound e: s.getEffectEndRound()) {
                ...
                checkBoolStrings(_data,_varsDiff,
                        _boolVarsDiffNotSending, e.getFailEndRound());
            }
            ...
            if (!fail_.isEmpty()) {
                ...
                //                if (Pattern.compile(_varRegExp).matcher(chBool_.toString()).find()) {
                    //                    throw new DataException();
                    //                }
                checkIfVarPresent(chBool_.beforeEvaluated());
        ...
        for (Item o: _data.getItems().values()) {
            if (o instanceof ItemForBattle) {
                ...
                if (!o_.getMultPower().isEmpty()) {
                    ...
                    String cat_ = _data.getMove(_data.getDefaultMove()).getCategory();
                    StringList types_ = _data.getMove(_data.getDefaultMove()).getTypes();
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTAQUE_CATEGORIE), cat_);
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LANCEUR_NOM), userFighterLoc_.getName());
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTAQUE_TYPES), types_.join(_data.getSepartorSetChar()));
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTAQUE_NOM), _data.getDefaultMove());
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.PUISSANCE_BASE), DataBase.getDefaultPower().toNumberString());
                ...
                if (!o_.getMultDamage().isEmpty()) {
                    ...
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTAQUE_NOM), _data.getDefaultMove());
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.PUISSANCE_BASE), DataBase.getDefaultPower().toNumberString());
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.COEFF_EFF), DataBase.defRateProduct().toNumberString());
                    loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.NB_UTILISATION_CONSECUTIF), userFighterLoc_.getNbRepeatingSuccessfulMoves().toNumberString());
                        ...
                        //                    case ACCURACY:
                            //                    case EVASINESS:
                                ...
                                //                    default:
                                    ...
                                    //                        loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTAQUE_CATEGORIE), cat_);
                                    //                        loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LANCEUR_NOM), userFighterLoc_.getName());
                    //                        loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTAQUE_TYPES), types_.join(NumericString.SEPARATOR_SET_CHAR));
                    //                        loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.ATTAQUE_NOM), _data.getDefaultMove());
                    //                        loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.PUISSANCE_BASE), DataBase.getDefaultPower().toString());
                    //                        loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.COEFF_EFF), DataBase.defRateProduct().toString());
                    //                        loc_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.NB_UTILISATION_CONSECUTIF), userFighterLoc_.getNbRepeatingSuccessfulMoves().toString());
                    //                        break;
                    //                    }
                ...
                for (String v: o_.getFailStatus().values()) {
                    ...
                    checkBoolStrings(_data,
                            _varsDiff, _boolVarsDiffNotSending, v);
                    checkBoolStrings(_data,_varsSame,
                            _boolVarsNotSending, v);
                }
                for (EffectEndRound e: o_.getEffectEndRound()) {
                    ...
                    checkBoolStrings(_data,_varsDiff,
                            _boolVarsDiffNotSending, e.getFailEndRound());
                    checkBoolStrings(_data,_varsSame,
                            _boolVarsNotSending, e.getFailEndRound());
                }
                for (EffectWhileSending e: o_.getEffectSending()) {
                    if (e instanceof EffectWhileSendingWithStatistic) {
                        ...
                        EqList<TeamPosition> listFighters_ = FightOrder.targetsEffect(_fight,_userFighter, e_, _diff, _data);
                        boolean containsTarget_ = listFighters_.containsObj(_foeFighter);
                        boolean containsThrower_ = listFighters_.containsObj(_userFighter);
                        if (containsTarget_) {
                            for (String f: fails_) {
                                if (f.isEmpty()) {
                                    continue;
    ...
    private static void checkMovesStr(DataBase _data,
            ...
            StringMap<String> _boolVarsNotSending,
            StringMap<String> _boolVarsDiffNotSending,
            StringMap<String> _boolVarsSending,
            StringMap<String> _varsSending) {
        NumericableString<Rate> chNum_;
        for (String m: _data.getMoves().getKeys()) {
            MoveData m_ = _data.getMove(m);
            for (TeamPosition t: FightOrder.targetsEffect(_fight,_userFighter, m_.getEffet(m_.indexOfPrimaryEffect()), _diff, _data)) {
                if (TeamPosition.eq(t, _userFighter)) {
                    chNum_ = _data.createNumericableString(m_.getAccuracy(), _varsSame);
                } else {
                    chNum_ = _data.createNumericableString(m_.getAccuracy(), _varsDiff);
                }
                //                if (Pattern.compile(_varRegExp).matcher(chNum_.toString()).find()) {
                //                    throw new DataException();
                //                }
                checkIfVarPresent(chNum_.beforeEvaluated());
            ...
            for (Effect e: m_.getEffects()) {
                ...
                for (TeamPosition t: FightOrder.targetsEffect(_fight,_userFighter, e, _diff, _data)) {
                    NumericableString<Boolean> chBool_;
                    if (TeamPosition.eq(t, _userFighter)) {
                        StringMap<String> map_ = new StringMap<String>();
            ...
            for (Effect e: m_.getEffects()) {
                ...
                if (eff_.getConstDamage()) {
                    ...
                    chNum_ = _data.createNumericableString(eff_.getPower(), _varsDiff);
                    //                    if (Pattern.compile(_varRegExp).matcher(chNum_.toString()).find()) {
...
//                    String bef_ = chNum_.beforeEvaluated();
            ...
            for (Effect e: m_.getEffects()) {
                ...
                checkBoolStrings(_data,
                        _varsSame, _boolVarsSending, fail_);
            }
            for (Effect e: m_.getEffects()) {
                if (!(e instanceof EffectTeamWhileSendFoe)) {
                    continue;
            ...
            for (Effect e: m_.getEffects()) {
                ...
                EqList<TeamPosition> listFighters_ = FightOrder.targetsEffect(_fight,_userFighter, e, _diff, _data);
                boolean containsTarget_ = listFighters_.containsObj(_foeFighter);
                boolean containsThrower_ = listFighters_.containsObj(_userFighter);
                if (containsTarget_) {
                    for (String f: fails_) {
                        ...
                        checkBoolStrings(_data,
                                _varsDiff, _boolVarsDiffNotSending, f);
                    }
                }
                if (containsThrower_) {
                    for (String f: fails_) {
                        ...
                        checkBoolStrings(_data,
                                _varsSame, _boolVarsNotSending, f);
                    }
                }
            }
            for (Effect e: m_.getEffects()) {
    ...
    private static void checkRateGrowLevel(DataBase _data, boolean _validateLevel) {
        if (_validateLevel) {
            ...
            for (long levelWin_ = minLevel_; levelWin_ < maxLevel_; levelWin_++) {
                for (long levelLoose_ = minLevel_; levelLoose_ < maxLevel_; levelLoose_++) {
                    for (DifficultyWinPointsFight d: DifficultyWinPointsFight.values()) {
                        ...
                        vars_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LEVEL_WINNER),Long.toString(levelWin_));
                        vars_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LEVEL_LOOSER),Long.toString(levelLoose_));
                        NumericableString<Rate> chNum_ = _data.createNumericableString(_data.getRates().getVal(d), vars_);
            ...
            for (DifficultyWinPointsFight d: DifficultyWinPointsFight.values()) {
                StringMap<String> vars_ = new StringMap<String>();
                vars_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LEVEL_WINNER),LgInt.one().toNumberString());
                vars_.put(StringList.concat(DataBase.VAR_PREFIX,Fight.LEVEL_LOOSER),LgInt.one().toNumberString());
                NumericableString<Rate> chNum_ = _data.createNumericableString(_data.getRates().getVal(d), vars_);
                chNum_.evaluateExp(true);
                if (!chNum_.isValid()) {
                    throw new DataException();
    ...
    private static void checkBoosts(DataBase _data) {
        ...
        for (long b = minBoost_; b <= maxBoost_; b++) {
            ...
            NumericableString<Rate> chNum_=_data.createNumericableString(rateBoost_, variables_);
            chNum_.evaluateExp(false);
    ...
    private static void checkExpGrowth(DataBase _data) {
        long minLevel_ = _data.getMinLevel();
        long maxLevel_ = _data.getMaxLevel();
        String varName_ = StringList.concat(DataBase.VAR_PREFIX,Fighter.NIVEAU);
        for (ExpType e: _data.getExpGrowth().getKeys()) {
            ...
            NumericableString<Rate> chNum_ = _data.createNumericableString(formula_, vars_);
            chNum_.evaluateExp(false);
    ...
    private static StringMap<String> calculateBooleanValuesForValidation(
        ...
        variables_.putAllMap(FightValues.calculateBasicBooleanValues(_fight,_lanceur,_cible,_import));
        return variables_;
    }

    private static void checkIfVarPresent(String _string) {
        if (_string.isEmpty()) {
            return;
        }
    ...
    private static void checkTranslations(DataBase _data, String _string) {
        ...
        String emptySet_ = StringList.concat(DataBase.EMPTY_STRING, String.valueOf(StringList.LEFT_BRACE), String.valueOf(StringList.RIGHT_BRACE));
        StringList sets_ = StringList.getTokensSets(StringList.removeStrings(_string, emptySet_));
        for (String s: sets_) {
            if (StringList.quickEq(s,emptySet_)) {
                continue;
            }
            if (!s.startsWith(StringList.concat(DataBase.EMPTY_STRING, String.valueOf(StringList.LEFT_BRACE)))) {
                continue;
            }
            String insideSet_ = s.substring(CustList.SECOND_INDEX, s.length() - 1);
            ...
            for (String w: words_) {
                if (!StringList.isWord(w)) {
                    ...
                    if (StringList.quickEq(w,String.valueOf(_data.getSepartorSetChar()))) {
                        continue;
                    }
                    throw new DataException();
                }
