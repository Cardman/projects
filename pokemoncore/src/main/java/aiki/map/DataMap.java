...
public final class DataMap {
        ...
        for (short p = CustList.FIRST_INDEX; p < nbPlaces_; p++) {
            ...
            if (places.getVal(p) instanceof Cave) {
                ...
                for (LevelPoint l : links_.getKeys()) {
                    ...
                    if (!Numbers.eq(other_.getLevel().getLevelIndex(),
                            l.getLevelIndex())) {
                        error = true;
                        return;
                    }
        ...
        for (short p_ : places.getKeys()) {
            ...
            for (byte l : pl_.getLevelsMap().getKeys()) {
                ...
                for (int index_ : levelPokemon_) {
                    ...
                    for (WildPk p : levelWild_.getWildPokemonAreas()
                            .get(index_).getWildPokemon()) {
                        if (!directCatchPk_.contains(p.getName())) {
                            directCatchPk_.put(p.getName(),
                                    new EnumList<Gender>(p.getGender()));
                        } else {
                            directCatchPk_.getVal(p.getName()).add(
                                    p.getGender());
                            directCatchPk_.getVal(p.getName())
                                    .removeDuplicates();
                        }
                    }
                    for (WildPk p : levelWild_.getWildPokemonAreas()
        ...
        for (short p = CustList.FIRST_INDEX; p < nbPlaces_; p++) {
            ...
            if (places.getVal(p) instanceof Cave) {
                ...
                for (LevelPoint l : links_.getKeys()) {
                    ...
                    Coords other_ = cave_.getLinksWithCaves().getVal(point_)
                            .getCoords();
                    if (!Numbers.eq(other_.getNumberPlace(), p)) {
                        error = true;
                        return;
    ...
    public void insertLinePlace(Coords _coords, short _nb) {
        ...
        if (p_ instanceof Cave) {
            ...
            ObjectMap<LevelPoint, Link> links_ = ((Cave) p_)
                    .getLinksWithOtherPlaces();
            ...
            for (Coords c : impactedPlaces_) {
                ...
                ObjectMap<Point, Link> linksLoc_ = ((InitializedPlace) place_)
                        .getLinksWithCaves();
                for (Point p : linksLoc_.getKeys()) {
                    Link l_ = linksLoc_.getVal(p);
                    Coords c_ = l_.getCoords();
                    if (!Numbers.eq(c_.getNumberPlace(),
                            _coords.getNumberPlace())) {
                        continue;
                    }
                    if (!Numbers.eq(c_.getLevel().getLevelIndex(), _coords
                            .getLevel().getLevelIndex())) {
                        continue;
                    }
            ...
            EqList<Coords> impactedPlaces_ = new EqList<Coords>();
            ObjectMap<Point, Link> links_ = ((InitializedPlace) p_)
                    .getLinksWithCaves();
            ...
            for (Coords c : impactedPlaces_) {
                ...
                ObjectMap<LevelPoint, Link> linksLoc_ = ((Cave) place_)
                        .getLinksWithOtherPlaces();
                for (LevelPoint p : linksLoc_.getKeys()) {
                    Link c_ = linksLoc_.getVal(p);
                    Coords coords_ = c_.getCoords();
                    if (!Numbers.eq(coords_.getNumberPlace(),
                            _coords.getNumberPlace())) {
                        continue;
                    }
                    if (!Numbers.eq(coords_.getLevel().getLevelIndex(), _coords
                            .getLevel().getLevelIndex())) {
                        continue;
                    }
    ...
    public void unjoin(short _pl1, short _pl2) {
        ...
        ObjectMap<PlaceInterConnect, Coords> join1_ = ((InitializedPlace) p1_)
                .getPointsWithCitiesAndOtherRoads();
    ...
    public void joinLevelCave(short _place, LevelPoint _l1, LevelPoint _l2,
    ...
    public void unjoinLevelCave(short _place, LevelPoint _l1) {
        ...
        l1_ = (LevelCave) cave_.getLevelsMap().getVal(
                link_.getCoords().getLevel().getLevelIndex());
    ...
    public void joinCavePlace(Coords _coordsCave, Coords _coordsPlace,
            String _imgName1, String _imgName2) {
        ...
        InitializedPlace place_ = (InitializedPlace) places.getVal(_coordsPlace
                .getNumberPlace());
    ...
    public void joinCavePlaceDirs(Coords _coordsCave, Coords _coordsPlace,
            String _imgName1, String _imgName2, Direction... _dirs) {
        if (!isEmptyForAdding(_coordsCave)) {
            return;
        }
        if (!isEmptyForAdding(_coordsPlace)) {
        ...
        if (_dirs.length == CustList.SIZE_EMPTY) {
            Cave cave_ = (Cave) places.getVal(_coordsCave.getNumberPlace());
            InitializedPlace place_ = (InitializedPlace) places
                    .getVal(_coordsPlace.getNumberPlace());
            ObjectMap<LevelPoint, Link> links1_ = cave_
                    .getLinksWithOtherPlaces();
            Link link1_ = new Link();
            link1_.setCoords(_coordsPlace);
            link1_.setFileName(_imgName1);
            ...
            if (!isEmptyForAdding(closestTile(_coordsPlace,
                    _dirs[CustList.FIRST_INDEX]))) {
                return;
            }
            Cave cave_ = (Cave) places.getVal(_coordsCave.getNumberPlace());
            InitializedPlace place_ = (InitializedPlace) places
            ...
            if (!isEmptyForAdding(closestTile(_coordsCave,
                    _dirs[CustList.FIRST_INDEX]))) {
                return;
            }
            if (!isEmptyForAdding(closestTile(_coordsPlace,
                    _dirs[CustList.SECOND_INDEX]))) {
                return;
            }
            Cave cave_ = (Cave) places.getVal(_coordsCave.getNumberPlace());
            InitializedPlace place_ = (InitializedPlace) places
    ...
    public void unjoinCavePlace(short _cave, LevelPoint _l1, short _place,
            LevelPoint _l2) {
    ...
    void unjoinCavePlace(short _cave, short _place) {
        ...
        for (LevelPoint p : keys2_) {
            if (Numbers.eq(join2_.getVal(p).getCoords().getNumberPlace(),
                    _place)) {
                join2_.removeKey(p);
            }
        }
    ...
    public boolean isEmptyForAdding(Coords _coords) {
        ...
        if (place_ instanceof League) {
            ...
            if (lPoint_.getLevelIndex() == CustList.FIRST_INDEX) {
                ...
                LevelLeague levelLeagueBack_ = (LevelLeague) ((League) place_)
                        .getLevelsMap().getVal(
        ...
        LevelPoint lPoint_ = _coords.getLevel();
        if (place_ instanceof Cave) {
            LevelCave levelCave_ = (LevelCave) ((Cave) place_).getLevelsMap()
    ...
    public void moveLink(Coords _link, Point _target) {
                ...
                if (lPoint_.getLevelIndex() > 0) {
                    LevelLeague levelLeagueBack_ = (LevelLeague) ((League) place_)
                            .getLevelsMap().getVal(
                                    (byte) (lPoint_.getLevelIndex() - 1));
                    if (Point
                            .eq(levelLeagueBack_.getNextLevelTarget(), _target)) {
                        return;
                    }
                } else {
        ...
        if (place_ instanceof City) {
            ...
            if (((City) place_).getLinksWithCaves().contains(pt_)) {
                ...
                Coords coords_ = link_.getCoords();
                Cave placeTarget_ = (Cave) places.getVal(coords_
                        .getNumberPlace());
                LevelPoint lPoint_ = coords_.getLevel();
                Link otherLink_;
                if (placeTarget_.getLinksWithOtherPlaces().contains(lPoint_)) {
                    otherLink_ = placeTarget_.getLinksWithOtherPlaces().getVal(
                            lPoint_);
                } else {
                    LevelCave levelCave_ = (LevelCave) placeTarget_
                            .getLevelsMap().getVal(lPoint_.getLevelIndex());
                    if (levelCave_.getLinksOtherLevels().contains(pt_)) {
                        otherLink_ = levelCave_.getLinksOtherLevels().getVal(
                                pt_);
                    } else {
                        return;
                    }
        ...
        if (place_ instanceof Cave) {
            ...
            if (levelCave_.getLinksOtherLevels().contains(pt_)) {
                ...
                LevelCave otherLevelCave_ = (LevelCave) ((Cave) place_)
                        .getLevelsMap().getVal(other_.getLevelIndex());
                Link otherLink_ = otherLevelCave_.getLinksOtherLevels().getVal(
                        other_.getPoint());
                otherLink_.getCoords().getLevel().getPoint().affect(_target);
            } else {
                if (((Cave) place_).getLinksWithOtherPlaces().contains(lPoint_)) {
                    link_ = ((Cave) place_).getLinksWithOtherPlaces().getVal(
                            lPoint_);
                    InitializedPlace init_ = (InitializedPlace) places
                            .getVal(link_.getCoords().getNumberPlace());
                    Link otherLink_ = init_.getLinksWithCaves().getVal(
                            link_.getCoords().getLevel().getPoint());
                    otherLink_.getCoords().getLevel().getPoint()
                            .affect(_target);
                } else {
                    return;
                }
    ...
    public void addPokemon(Coords _coords, WildPk _pk) {
        Campaign initPlace_ = (Campaign) places
                .getVal(_coords.getNumberPlace());
    ...
    public void removeLink(Coords _link) {
        ...
        if (place_ instanceof Cave) {
            ...
            if (levelCave_.getLinksOtherLevels().contains(pt_)) {
                ...
                LevelCave otherLevelCave_ = (LevelCave) ((Cave) place_)
                        .getLevelsMap().getVal(other_.getLevelIndex());
                otherLevelCave_.getLinksOtherLevels().removeKey(
                        other_.getPoint());
            } else {
                if (((Cave) place_).getLinksWithOtherPlaces().contains(lPoint_)) {
                    link_ = ((Cave) place_).getLinksWithOtherPlaces().getVal(
                    ...
                    InitializedPlace init_ = (InitializedPlace) places
                            .getVal(link_.getCoords().getNumberPlace());
                    init_.getLinksWithCaves().removeKey(
                            link_.getCoords().getLevel().getPoint());
                } else {
                    return;
                }
    ...
    public void deleteCave(short _cave) {
        ...
        for (short p = CustList.FIRST_INDEX; p < nbPlaces_; p++) {
            ...
            if (Numbers
                    .eq(((League) place_).getAccessCoords().getNumberPlace(),
                            _cave)) {
    ...
    public void deletePlace(short _place) {
        ...
        for (short p = CustList.FIRST_INDEX; p < nbPlaces_; p++) {
            ...
            if (Numbers.eq(
                    ((League) place_).getAccessCoords().getNumberPlace(),
                    _place)) {
                removeAccessLeague(p);
            }
        }
