...
//            }
            ...
            if (!Point.eq(access_.getInsideBuilding(), _coords.getInsideBuilding())) {
                continue;
            }
            if (!Numbers.eq(access_.getLevel().getLevelIndex(), _coords.getLevel().getLevelIndex())) {
                continue;
            }
            Point pt_ = access_.getLevel().getPoint();
            frontTiles_.put(new Point(pt_), _data.getLink(((League)p).getFileName()));
        }
        if (lev_ instanceof LevelLeague) {
            LevelLeague lv_ = (LevelLeague) lev_;
            Person person_ = lv_.getTrainer();
            frontTiles_.put(new Point(lv_.getTrainerCoords()), _data.getPerson(person_.getImageMiniFileName()));
            frontTiles_.put(new Point(lv_.getAccessPoint()), _data.getLink(lv_.getFileName()));
        }
        if (lev_ instanceof LevelWithWildPokemon) {
            ...
            for (Point p: lv_.getCharacters().getKeys()) {
                ...
                frontTiles_.put(new Point(p), _data.getPerson(person_.getImageMiniFileName()));
            }
            for (Point p: lv_.getLegendaryPks().getKeys()) {
                WildPk pk_ = lv_.getPokemon(p);
                frontTiles_.put(new Point(p), _data.getMiniPk().getVal(pk_.getName()));
            }
            for (Point p: lv_.getItems().getKeys()) {
                frontTiles_.put(new Point(p), _data.getMiniItems().getVal(lv_.getItems().getVal(p)));
            }
            ...
            for (Point p: lv_.getDualFights().getKeys()) {
                ...
                frontTiles_.put(new Point(p), _data.getPerson(tmp_.getImageMiniFileName()));
            }
            for (DualFight d: lv_.getDualFights().values()) {
                TempTrainer tmp_ = d.getFoeTrainer();
                frontTiles_.put(new Point(d.getPt()), _data.getPerson(tmp_.getImageMiniSecondTrainerFileName()));
            }
        }
        if (_coords.isInside()) {
            City city_ = (City) pl_;
            Building building_ = city_.getBuildings().getVal(_coords.getInsideBuilding());
            frontTiles_.put(new Point(building_.getExitCity()), _data.getLink(building_.getImageFileName()));
        }
        if (lev_ instanceof LevelIndoorPokemonCenter) {
            ...
            frontTiles_.put(new Point(lv_.getStorageCoords()), _data.getStorage());
        }
        if (lev_ instanceof LevelIndoorGym) {
            ...
            frontTiles_.put(new Point(lv_.getGymLeaderCoords()), _data.getPerson(person_.getImageMiniFileName()));
        }
        if (pl_ instanceof Cave) {
            Cave cave_ = (Cave) pl_;
            for (LevelPoint lp_: cave_.getLinksWithOtherPlaces().getKeys()) {
                if (!Numbers.eq(lp_.getLevelIndex(), _coords.getLevel().getLevelIndex())) {
                    continue;
                }
                Link link_ = cave_.getLinksWithOtherPlaces().getVal(lp_);
                frontTiles_.put(new Point(lp_.getPoint()),_data.getLink(link_.getFileName()));
            }
            LevelCave lv_ = (LevelCave) lev_;
            for (Point p: lv_.getLinksOtherLevels().getKeys()) {
                Link link_ = lv_.getLinksOtherLevels().getVal(p);
                frontTiles_.put(new Point(p),_data.getLink(link_.getFileName()));
            }
        }
...
//        }
    ...
    public void addLinesForSelectedBlock(Point _pt, int _nbLines) {
        ...
        if (!canContainsNewBlock(new Point(_pt.getx(),(short)(_pt.gety()+h_)), virtualBlock_)) {
            return;
        }
    ...
    public void addColumnsForSelectedBlock(Point _pt, int _nbColumns) {
        ...
        if (!canContainsNewBlock(new Point((short)(_pt.getx()+w_),_pt.gety()), virtualBlock_)) {
            return;
        }
    ...
    void changeImage(Point _pt, String _imageName, short _newWidth, short _newHeight) {
        Block selectedBlock_ = blocks.getVal(_pt);
    ...
    public boolean canMoveBlock(Point _id, Point _dest) {
        ...
        for(Point p:blocks.getKeys()){
            ...
            BlockBounds coinsBord_=Block.intersection(block_.bounds(p),blockToBeMoved_.bounds(_dest));
    ...
    public boolean canContainsNewBlock(Point _point, Block _block) {
        for(Point p:blocks.getKeys()){
            Block block_ = blocks.getVal(p);
            BlockBounds coinsBord_=Block.intersection(block_.bounds(p),_block.bounds(_point));
            if(coinsBord_.isValid()){
                return false;
            }
        }
